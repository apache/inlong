...
public class ManagerIdTopicConfigLoader implements IdTopicConfigLoader {
    ...
    public List<IdTopicConfig> load() {
        ...
        DataProxyCluster dataProxyCluster =
                RemoteConfigManager.getInstance().getCurrentClusterConfig();
        ...
        for (InLongIdObject obj : dataProxyCluster.getProxyCluster().getInlongIds()) {
            ...
            config.setDataType(
                    DataType.convert(params.getOrDefault("dataType", DataType.TEXT.value())));
