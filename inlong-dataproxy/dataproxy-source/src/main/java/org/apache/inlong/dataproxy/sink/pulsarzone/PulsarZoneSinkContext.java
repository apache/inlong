...
public class PulsarZoneSinkContext extends SinkContext {
    ...
    public PulsarZoneSinkContext(
        ...
        String strCompressionType =
                CommonPropertiesHolder.getString(
                        KEY_COMPRESS_TYPE, INLONG_COMPRESSED_TYPE.INLONG_SNAPPY.name());
     ...
     * @param sendTime
     */
    public void processSendFail(
            DispatchProfile currentRecord, String producerTopic, long sendTime) {
    ...
    public void addSendResultMetric(
        ...
        long auditFormatTime =
                dispatchTime - dispatchTime % CommonPropertiesHolder.getAuditFormatInterval();
        dimensions.put(DataProxyMetricItem.KEY_MESSAGE_TIME, String.valueOf(auditFormatTime));
        DataProxyMetricItem metricItem = this.getMetricItemSet().findMetricItem(dimensions);
        long count = currentRecord.getCount();
        long size = currentRecord.getSize();
