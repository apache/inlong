...
public class InlongTcpChannelPipelineFactory extends ChannelInitializer<SocketChannel>
    ...
    protected void initChannel(SocketChannel ch) {
        ...
        if (StringUtils.isEmpty(protocolType)
                || this.protocolType.equalsIgnoreCase(ConfigConstants.TCP_PROTOCOL)) {
            ch.pipeline()
                    .addLast(
                            "messageDecoder",
        ...
        if (sourceContext.getSource().getChannelProcessor() != null) {
            try {
                ...
                ChannelInboundHandlerAdapter messageHandler =
                        (ChannelInboundHandlerAdapter) ctor.newInstance(sourceContext);

                ch.pipeline().addLast("messageHandler", messageHandler);
            } catch (Exception e) {
                LOG.error(
                        "SimpleChannelHandler.newInstance  has error:"
                                + sourceContext.getSource().getName(),
                        e);
            }
        }
    }
