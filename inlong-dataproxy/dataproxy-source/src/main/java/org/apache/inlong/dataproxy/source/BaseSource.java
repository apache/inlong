...
public abstract class BaseSource extends AbstractSource implements EventDrivenSource, Configurable {
    ...
    public void configure(Context context) {
        ...
        Preconditions.checkArgument(
                (statIntervalSec >= STAT_INTERVAL_MUST_THAN), "statIntervalSec must be >= 0");

        pkgTimeoutSec =
                context.getInteger(ConfigConstants.PACKAGE_TIMEOUT_SEC, PKG_TIMEOUT_DEFAULT_SEC);
