        ...
        try {
            ...
            if (sslEnabled) {
                ...
                ServerConnector http2Connector =
                        new ServerConnector(srv, ssl, new HttpConnectionFactory(httpsConfig));
            ...
            servletContext.addFilter(
                    new FilterHolder(new MessageFilter(maxMsgLength)),
                    "/dataproxy/*",
                    EnumSet.of(DispatcherType.REQUEST));
            servletContext.addServlet(
                    new ServletHolder(new MessageProcessServlet(messageHandler)), "/dataproxy/*");
            servletContext.addServlet(
                    new ServletHolder(new ConfigMessageServlet()), "/dataproxy/config/*");
            srv.start();
