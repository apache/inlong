...
public class PulsarProducerCluster implements LifecycleAware {
    ...
    public PulsarProducerCluster(
            String workerName, CacheClusterConfig config, PulsarFederationSinkContext context) {
    ...
    public void start() {
        ...
        try {
            ...
            this.baseBuilder.batchingMaxPublishDelay(
                    context.getInteger(KEY_BATCHINGMAXPUBLISHDELAY, 100), TimeUnit.MILLISECONDS);
            this.baseBuilder.batchingMaxBytes(context.getInteger(KEY_BATCHINGMAXBYTES, 131072));
            this.baseBuilder
    ...
    public boolean send(Event event) {
        ...
        if (producer == null) {
            try {
                ...
                producer = baseBuilder.clone().topic(topic).producerName(producerName).create();
        ...
        if (messageKey == null) {
            future = producer.newMessage().properties(headers).value(event.getBody()).sendAsync();
        } else {
            future =
                    producer.newMessage()
                            .key(messageKey)
