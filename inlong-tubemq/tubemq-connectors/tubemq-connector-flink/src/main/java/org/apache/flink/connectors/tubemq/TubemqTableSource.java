...
public class TubemqTableSource
    ...
    public TubemqTableSource(
        ...
        checkNotNull(schema, "The schema must not be null.");
        checkNotNull(fieldMapping, "The field mapping must not be null.");
    ...
    public DataStream<Row> getDataStream(StreamExecutionEnvironment streamExecutionEnvironment) {
        ...
        return streamExecutionEnvironment.addSource(sourceFunction).name(explainSource());
    }
                ...
                (attribute) -> {
                    ...
                    if (!tpe.isPresent()) {
                        throw new ValidationException(
                                "Proc time attribute '"
                                        + attribute
                                        + "' isn't present in TableSchema.");
                    } else if (tpe.get() != Types.SQL_TIMESTAMP()) {
                        throw new ValidationException(
                                "Proc time attribute '"
                                        + attribute
                                        + "' isn't of type SQL_TIMESTAMP.");
                    }
                    return attribute;
                });
    }
    ...
    private List<RowtimeAttributeDescriptor> validateRowTimeAttributeDescriptors(
        ...
        for (RowtimeAttributeDescriptor desc : attributeDescriptors) {
            ...
            if (!tpe.isPresent()) {
                ...
                throw new ValidationException(
                        "Row time attribute '" + name + "' is not of type SQL_TIMESTAMP.");
            }
        }
