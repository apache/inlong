...
public class RoundRobinPartitionRouter implements PartitionRouter {
    ...
    @Override
    public Partition getPartition(final Message message, final List<Partition> partitions)
        ...
        for (int i = 0; i < partSize; i++) {
            roundPartition =
                    partitions.get(
                            (currRouterCount.incrementAndGet() & Integer.MAX_VALUE) % partSize);
            if (roundPartition != null
                    && roundPartition.getDelayTimeStamp() < System.currentTimeMillis()) {
