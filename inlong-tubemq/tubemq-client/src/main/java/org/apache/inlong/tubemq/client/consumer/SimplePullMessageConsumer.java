...
public class SimplePullMessageConsumer implements PullMessageConsumer {
    ...
    public SimplePullMessageConsumer(
            final InnerSessionFactory messageSessionFactory, final ConsumerConfig consumerConfig)
            throws TubeClientException {
        baseConsumer = new BaseMessageConsumer(messageSessionFactory, consumerConfig, true);
    }
    ...
    @Override
    public PullMessageConsumer subscribe(String topic, TreeSet<String> filterConds)
            throws TubeClientException {
        ...
        while (true) {
            if (baseConsumer.isShutdown()) {
                return new ConsumerResult(
                        TErrCodeConstants.BAD_REQUEST, "Client instance has been shutdown!");
            }
    ...
    @Override
    public ConsumerResult confirmConsume(final String confirmContext, boolean isConsumed)
            throws TubeClientException {
        baseConsumer.checkClientRunning();
        if (!baseConsumer.isSubscribed()) {
            throw new TubeClientException("Please complete topic's Subscribe call first!");
        ...
        Partition curPartition = baseConsumer.rmtDataCache.getPartitionByKey(keyId);
            ...
            try {
                ClientBroker.CommitOffsetResponseB2C commitResponse =
                        baseConsumer
                                ...
                                .consumerCommitC2B(
                                        baseConsumer.createBrokerCommitRequest(
                                                curPartition, isConsumed),
                ...
                if (commitResponse == null) {
                    ...
                    return new ConsumerResult(
                            ...
                            commitResponse.getErrCode(),
                            commitResponse.getErrMsg(),
                            topicName,
                            curPartition,
                            currOffset,
                            maxOffset);
                ...
                baseConsumer.clientStatsInfo.bookConfirmDuration(
                        keyId, System.currentTimeMillis() - midTime);
            }
        }
    }
}
