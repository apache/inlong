...
public interface ClientBalanceConsumer extends Shutdownable {
     ...
     *
     * @param topicAndFilterCondMap
     *          subscribed topic and need filtered condition items of topic if not
     *          need filter consume messages, set condition's set is empty or null
     * @param sourceCount
     *          the total count of clients that the consumer group will start this
     *          time If this value is set, the system will check that this parameter
     *          value carried by each client must be consistent, and the
     *          corresponding nodeId value must be unique in the consumer group; if
     *          this value is not set, Please set a negative number
     * @param nodeId
     *          the unique ID of the node in the consumer group Attention:The
     *          sourceCount and nodeId parameters are used when the client performs
     *          modular allocation of partitions to avoid allocation conflicts in
     *          advance and avoid repeated allocations to the same partition by the
     *          client
     * @param result
     *          call result, the parameter is not allowed to be null
    ...
    boolean start(Map<String, TreeSet<String>> topicAndFilterCondMap,
            int sourceCount, int nodeId, ProcessResult result)
            throws TubeClientException;
     ...
     *          boostrap offset for consumption, if value: < 0, broker will not
     *          change stored offset, >= 0, broker will replace the stored offset
     ...
     *          confirm context, from the corresponding field value in the result of
     *          getting the message
     * @param isConsumed
     *          whether the data has been successfully processed, if true, tell the
     *          server to continue processing the next batch of data; false, tell
     *          the server that the data has not been processed successfully and
     *          need to be re-pulled for processing
     * @throws TubeClientException
     *           parameter abnormal
     *
     * @return true if call success, false if failure
