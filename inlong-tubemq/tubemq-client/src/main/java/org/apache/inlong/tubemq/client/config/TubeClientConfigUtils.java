...
public class TubeClientConfigUtils {
     ...
     */
    public static RpcConfig getRpcConfigByClientConfig(
            final TubeClientConfig tubeClientConfig, boolean isSingleSession) {
        RpcConfig config = new RpcConfig();
        config.put(RpcConstants.TLS_OVER_TCP, tubeClientConfig.isTlsEnable());
        if (tubeClientConfig.isTlsEnable()) {
            config.put(RpcConstants.TLS_TRUSTSTORE_PATH, tubeClientConfig.getTrustStorePath());
            config.put(
                    RpcConstants.TLS_TRUSTSTORE_PASSWORD, tubeClientConfig.getTrustStorePassword());
            config.put(
                    RpcConstants.TLS_TWO_WAY_AUTHENTIC,
                    tubeClientConfig.isEnableTLSTwoWayAuthentic());
            if (tubeClientConfig.isEnableTLSTwoWayAuthentic()) {
                config.put(RpcConstants.TLS_KEYSTORE_PATH, tubeClientConfig.getKeyStorePath());
                config.put(
                        RpcConstants.TLS_KEYSTORE_PASSWORD, tubeClientConfig.getKeyStorePassword());
            }
        }
