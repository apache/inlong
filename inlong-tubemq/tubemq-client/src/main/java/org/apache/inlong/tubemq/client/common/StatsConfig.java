...
public class StatsConfig {
    ...
    public void updateStatsConfig(StatsConfig that) {
        ...
        updateStatsConfig(
                that.statsLevel,
                that.enableSelfPrint,
                that.selfPrintPeriodMs,
                that.forcedResetPeriodMs);
    }

    /**
     * Update current configure settings Attention, if statsLevel is ZERO, then printing will be
     * automatically turned off regardless of the value of enableSelfPrint
     *
    ...
    /**
     * Update current statistics control settings Attention, if statsLevel is ZERO, then printing
     * will be automatically turned off regardless of the value of enableSelfPrint
     *
     * @param statsLevel the statistics level
     * @param enableSelfPrint whether to allow the SDK to print by itself
     */
    ...
    public void setStatsPeriodInfo(long selfPrintPeriodMs, long forcedResetPeriodMs) {
        this.selfPrintPeriodMs =
                MixedUtils.mid(
                        selfPrintPeriodMs,
                        STATS_SELF_PRINT_MIN_PERIOD_MS,
                        STATS_SELF_PRINT_MAX_PERIOD_MS);
        this.forcedResetPeriodMs =
                MixedUtils.mid(
                        forcedResetPeriodMs,
                        STATS_AUTO_RESET_MIN_PERIOD_MS,
                        STATS_AUTO_RESET_MAX_PERIOD_MS);
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(statsLevel, enableSelfPrint, selfPrintPeriodMs, forcedResetPeriodMs);
    }

    @Override
    public String toString() {
        return new StringBuilder(512)
                .append("\"StatsConfig\":{\"statsLevel\":\"")
                .append(statsLevel.getName())
                .append("\",\"enableSelfPrint\":")
                .append(enableSelfPrint)
                .append(",\"selfPrintPeriodMs\":")
                .append(selfPrintPeriodMs)
                .append(",\"forcedResetPeriodMs\":")
                .append(forcedResetPeriodMs)
                .append("}")
                .toString();
    }
