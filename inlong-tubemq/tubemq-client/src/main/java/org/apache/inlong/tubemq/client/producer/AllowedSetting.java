...
/** The class class caches the dynamic settings returned from the server. */
public class AllowedSetting {
    private AtomicLong configId = new AtomicLong(TBaseConstants.META_VALUE_UNDEFINED);
    private AtomicInteger maxMsgSize = new AtomicInteger(TBaseConstants.META_MAX_MESSAGE_DATA_SIZE);
    ...
    public void updAllowedSetting(ClientMaster.ApprovedClientConfig allowedConfig) {
        if (allowedConfig != null) {
            ...
            if (allowedConfig.hasMaxMsgSize()
                ...
                maxMsgSize.set(
                        SettingValidUtils.validAndGetMaxMsgSizeInB(allowedConfig.getMaxMsgSize()));
            }
        }
    }
