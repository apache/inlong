 ...
 *
 * <p>This class performs the index statistics of the time period with the exponential power of 2.
 * Here we count up to the 17th power of 2, a total of 18 buckets, according to the input data and
 * the corresponding time index, the corresponding cell is selected for statistics. When outputting
 * data, the full statistical output or only the output is not 0 according to different requirements
 */
public class ESTHistogram extends BaseMetric implements Histogram {
    ...
    public ESTHistogram(String metricName, String prefix) {
        ...
        for (int i = 0; i < NUM_BUCKETS; i++) {
            ...
            if (i == 0) {
                ...
                strBuff.append((int) Math.pow(2, i)).append("t").append((int) Math.pow(2, i + 1));
            }
    ...
    public void getValue(StringBuilder strBuff, boolean includeZero) {
        strBuff.append("\"")
                ...
                .append("\":")
                .append("{\"")
                .append(this.count.getShortName())
                .append("\":")
                .append(this.count.getValue())
                .append(",\"")
                .append(this.min.getShortName())
                .append("\":")
                .append(this.min.getValue())
                .append(",\"")
                .append(this.max.getShortName())
                .append("\":")
                .append(this.max.getValue())
                .append(",\"cells\":{");
        int count = 0;
        if (includeZero) {
            for (int i = 0; i < NUM_BUCKETS; i++) {
                ...
                strBuff.append("\"")
                        .append(this.buckets[i].getShortName())
                        .append("\":")
    ...
    public void snapShort(StringBuilder strBuff, boolean includeZero) {
        strBuff.append("\"")
                ...
                .append(this.count.getShortName())
                .append("\":")
                .append(this.count.getAndResetValue())
                .append(",\"")
                .append(this.min.getShortName())
                .append("\":")
                .append(this.min.getAndResetValue())
                .append(",\"")
                .append(this.max.getShortName())
                .append("\":")
                .append(this.max.getAndResetValue())
                .append(",\"cells\":{");
        int count = 0;
        if (includeZero) {
            for (int i = 0; i < NUM_BUCKETS; i++) {
