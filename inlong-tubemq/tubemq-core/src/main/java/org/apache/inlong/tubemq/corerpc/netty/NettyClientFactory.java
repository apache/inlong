...
/**
 * Network communication between service processes based on netty see @link MessageSessionFactory
...
public class NettyClientFactory implements ClientFactory {
    ...
    public void configure(final RpcConfig conf) throws IllegalArgumentException {
        if (this.shutdown.compareAndSet(true, false)) {
            ...
            final int workerCount =
                    conf.getInt(
                            RpcConstants.WORKER_COUNT,
                            RpcConstants.CFG_DEFAULT_CLIENT_WORKER_COUNT);
            String threadName =
                    new StringBuilder(256)
                            .append(
                                    conf.getString(
                                            RpcConstants.WORKER_THREAD_NAME,
                                            RpcConstants.CFG_DEFAULT_WORKER_THREAD_NAME))
                            .append(workerIdCounter.incrementAndGet())
                            .toString();
            eventLoopGroup =
                    EventLoopUtil.newEventLoopGroup(
                            ...
                            new DefaultThreadFactory(
                                    threadName, Thread.currentThread().isDaemon()));
        }
    }
    ...
    private Client createClient(
            final NodeAddrInfo addressInfo, int connectTimeout, final RpcConfig conf)
            throws Exception {
                    ...
                    public void initChannel(SocketChannel socketChannel) throws Exception {
                            ...
                            try {
                                ...
                                logger.error(
                                        new StringBuilder(256)
                                                ...
                                                .append(addressInfo.getHostPortStr())
                                                .append(" failure!")
                                                .toString(),
                                        t);
                        ...
                        pipeline.addLast(
                                ...
                                new ReadTimeoutHandler(
                                        conf.getLong(
                                                RpcConstants.CONNECT_READ_IDLE_DURATION,
        ...
        ChannelFuture future =
                clientBootstrap.connect(
                        new InetSocketAddress(addressInfo.getHost(), addressInfo.getPort()));
        future.awaitUninterruptibly(connectTimeout);
        if (!future.isDone()) {
            future.cancel(false);
            throw new LocalConnException(
                    new StringBuilder(256)
                            .append("Create connection to ")
                            .append(addressInfo.getHostPortStr())
                            .append(" timeout!")
                            .toString());
        }
