...
public class PbEnDecoder {
    ...
    static {
        // The MAP corresponding to the writing of these strings and constants when the system
        // starts up
        ...
        rpcMethodMap.put(
                "producerCloseClientP2M", RpcConstants.RPC_MSG_MASTER_PRODUCER_CLOSECLIENT);
        ...
        rpcMethodMap.put(
                "consumerCloseClientC2M", RpcConstants.RPC_MSG_MASTER_CONSUMER_CLOSECLIENT);
        ...
        rpcMethodMap.put("consumerRegisterC2MV2", RpcConstants.RPC_MSG_MASTER_CONSUMER_REGISTER_V2);
        rpcMethodMap.put(
                "consumerHeartbeatC2MV2", RpcConstants.RPC_MSG_MASTER_CONSUMER_HEARTBEAT_V2);
        rpcMethodMap.put(
                "consumerGetPartMetaInfoC2M", RpcConstants.RPC_MSG_MASTER_CONSUMER_GET_PART_META);

        rpcServiceMap.put(
                "org.apache.inlong.tubemq.corerpc.service.MasterService",
                RpcConstants.RPC_SERVICE_TYPE_MASTER_SERVICE);
        rpcServiceMap.put(
                "org.apache.inlong.tubemq.corerpc.service.BrokerReadService",
                RpcConstants.RPC_SERVICE_TYPE_BROKER_READ_SERVICE);
        rpcServiceMap.put(
                "org.apache.inlong.tubemq.corerpc.service.BrokerWriteService",
                RpcConstants.RPC_SERVICE_TYPE_BROKER_WRITE_SERVICE);
        rpcServiceMap.put(
                "org.apache.inlong.tubemq.corerpc.service.BrokerWriteService$AsyncService",
                RpcConstants.RPC_SERVICE_TYPE_BROKER_WRITE_SERVICE);
    }
    ...
    public static Object pbDecode(boolean isRequest, int methodId, byte[] bytes) throws Exception {
        // #lizard forgives
        // According to the method ID carried in the pb message, the corresponding class is directly
        // used for mapping.
        if (isRequest) {
            switch (methodId) {
    ...
    public static int getMethIdByName(String methodName) throws Exception {
        ...
        if (methodId == null) {
            throw new Exception(
                    new StringBuilder(256)
                            .append("Unsupported method name ")
                            .append(methodName)
                            .toString());
        } else {
            return methodId;
    ...
    public static int getServiceIdByServiceName(String serviceName) throws Exception {
        ...
        if (serviceId == null) {
            throw new Exception(
                    new StringBuilder(256)
                            .append("Unsupported service name ")
                            .append(serviceName)
                            .toString());
        } else {
            return serviceId;
    ...
    public static boolean isValidServiceTypeAndMethod(
            int serviceId, int methodId, final StringBuilder sBuilder) throws Exception {
        // #lizard forgives
            ...
            case RpcConstants.RPC_SERVICE_TYPE_MASTER_SERVICE:
                {
                    switch (methodId) {
                        ...
                        default:
                            {
                                if (sBuilder != null) {
                                    sBuilder.append("Unsupported method ")
                                            .append(methodId)
                                            .append("in service type ")
                                            .append(serviceId)
                                            .append("!");
                                }
                                return false;
