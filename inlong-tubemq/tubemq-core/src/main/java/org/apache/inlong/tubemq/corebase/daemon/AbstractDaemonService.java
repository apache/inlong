...
public abstract class AbstractDaemonService implements Service, Runnable {
    ...
    public void run() {
        logger.info(
                new StringBuilder(256).append(name).append("-daemon-thread started").toString());
        this.loopProcess(this.intervalMs);
        logger.info(
                new StringBuilder(256).append(name).append("-daemon-thread stopped").toString());
    }
