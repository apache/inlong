...
public class RpcProtocol implements Protocol {
    ...
    public void registerService(
            boolean isOverTLS, String serviceName, Object instance, ExecutorService threadPool)
            throws Exception {
    ...
    @Override
    public void handleRequest(final RequestContext context, final String rmtAddress)
            throws Exception {
        ...
        if (System.currentTimeMillis() - context.getReceiveTime() > requestWrapper.getTimeout()) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Timeout when request arrived, so give up processing this request from : {}",
                        rmtAddress);
            }
            return;
        }
        if (ServiceStatusHolder.isServiceStopped()) {
            context.write(
                    new ResponseWrapper(
                            RpcConstants.RPC_FLAG_MSG_TYPE_RESPONSE,
                            requestWrapper.getSerialNo(),
                            requestWrapper.getServiceType(),
                            requestWrapper.getProtocolVersion(),
                            RPC_PROTOCOL_VERSION,
                            new ServiceStoppingException("service is stopping...")));
        }
        Method method = null;
        StringBuilder sBuilder = new StringBuilder(512);
        try {
            if (!PbEnDecoder.isValidServiceTypeAndMethod(
                    requestWrapper.getServiceType(), requestWrapper.getMethodId(), sBuilder)) {
                throw new Exception(sBuilder.toString());
            }
            Object processor = processors.get(requestWrapper.getServiceType());
            if (processor == null) {
                throw new Exception(
                        sBuilder.append("No service ")
                                .append(requestWrapper.getServiceType())
                                .append(" found on the server")
                                .toString());
            }
            method = cacheMethods.get(requestWrapper.getMethodId());
            if (method == null) {
                throw new Exception(
                        sBuilder.append("No method ")
            ...
            Object result =
                    method.invoke(
                            processor, requestWrapper.getRequestData(), rmtAddress, isOverTLS);
            responseWrapper =
                    new ResponseWrapper(
                            RpcConstants.RPC_FLAG_MSG_TYPE_RESPONSE,
                            requestWrapper.getSerialNo(),
            ...
            errorClass =
                    MixUtils.replaceClassNamePrefix(
                            errorClass, true, requestWrapper.getProtocolVersion());
            responseWrapper =
                    new ResponseWrapper(
                            RpcConstants.RPC_FLAG_MSG_TYPE_RESPONSE,
                            requestWrapper.getSerialNo(),
