...
/**
 * This demo shows how to consume messages with bootstrap offset through TubeMultiSessionFactory +
 * PullMessageConsumer
 *
 * <p>First, we start a consumer for data consumption, and then we exit it, start another consumer
 * with the same group name, and set the partition's bootstrap offset to 0 for data consumption.
 *
 * <p>The main difference from {@link MessagePullConsumerExample} is that we call {@link
 * PullMessageConsumer#completeSubscribe(String, int, boolean, Map)} instead of {@link
 * PullMessageConsumer#completeSubscribe()}. The former supports multiple options to configure when
 * to reset offset.
 */
public final class MessagePullSetConsumerExample {
    ...
    /**
     * Consume messages with setting bootstrap offset in Pull mode through multi-session factory
     * instances.
     *
    ...
    public static void main(String[] args) throws Throwable {
        ...
        PullMessageConsumer consumer1 = createPullConsumer(masterServers, groupName);
        ...
        ConcurrentHashMap<String, Long> partOffsetMapFst = new ConcurrentHashMap<>();
        ...
        for (int i = 0; i < fetchRunners1.length; i++) {
            fetchRunners1[i] =
                    new Thread(
                            new FetchRequestRunner(consumer1, partOffsetMapFst, consumeCount),
                            "_fetch_runner_" + i);
        }
        ...
        ConcurrentHashMap<String, Long> partOffsetMapSec = new ConcurrentHashMap<>();
        for (String partKey : partOffsetMapFst.keySet()) {
            partOffsetMapSec.put(partKey, 0L);
        }
        // 4.2. build consumer object
        PullMessageConsumer consumer2 = createPullConsumer(masterServers, groupName);
        ...
        consumer2.completeSubscribe(sessionKeySec, sourceCountSec, isSelectBig, partOffsetMapSec);
        ...
        for (int i = 0; i < fetchRunners2.length; i++) {
            fetchRunners2[i] =
                    new Thread(
                            new FetchRequestRunner(consumer2, partOffsetMapSec, consumeCount),
                            "_fetch_runner_" + i);
        }
    ...
    private static PullMessageConsumer createPullConsumer(
        ...
        //    find and initialize TubeMultiSessionFactory object according to the Master cluster
        // information
    ...
    private static class FetchRequestRunner implements Runnable {
        ...
        FetchRequestRunner(
                PullMessageConsumer messageConsumer,
            ...
            do {
                try {
                    ...
                    if (csmResult.isSuccess()) {
                        ...
                        //       the offset returned by GetMessage() represents the initial offset
                        // of this request
                        //       if consumer group is pure Pull mode, the initial offset can be
                        // saved;
