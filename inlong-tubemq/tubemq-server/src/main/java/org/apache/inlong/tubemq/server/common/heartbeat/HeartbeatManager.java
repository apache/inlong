...
public class HeartbeatManager {
     ...
     */
    public TimeoutInfo regConsumerNode(
            final String nodeId, final String consumerId, final String partStr) {
        return this.consumerRegMap.put(
                nodeId, new TimeoutInfo(consumerId, partStr, this.consumerTimeoutDlt));
    }
     ...
     */
    public boolean updBrokerNode(
            String nodeId, String createId, StringBuilder sBuffer, ProcessResult result) {
        TimeoutInfo timeoutInfo = brokerRegMap.get(nodeId);
        if (timeoutInfo == null) {
            result.setFailResult(
                    TErrCodeConstants.HB_NO_NODE,
                    sBuffer.append("Invalid node id:")
                            .append(nodeId)
                            .append(", you have to append node first!")
                            .toString());
        ...
        if (!createId.equals(timeoutInfo.getSecondKey())) {
            result.setFailResult(
                    TErrCodeConstants.HB_NO_NODE,
                    sBuffer.append("Invalid node block id:")
                            .append(nodeId)
                            .append(", you have to append node first!")
                            .toString());
            sBuffer.delete(0, sBuffer.length());
