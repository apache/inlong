...
public class WebGroupResCtrlHandler extends AbstractWebHandler {
    ...
    public void registerWebApiMethod() {
        ...
        registerModifyWebMethod(
                "admin_batch_add_group_resctrl_info", "adminBatchAddGroupResCtrlConf");
        registerModifyWebMethod("admin_update_group_resctrl_info", "adminModGroupResCtrlConf");
        registerModifyWebMethod(
                "admin_batch_update_group_resctrl_info", "adminBatchUpdGroupResCtrlConf");
     ...
     */
    public StringBuilder adminQueryGroupResCtrlConf(
        ...
        if (!WebParameterUtils.getBooleanParamValue(
                req, WebFieldDef.RESCHECKENABLE, false, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return sBuffer;
        }
     ...
     * @return process result
     */
    public StringBuilder adminAddGroupResCtrlConf(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
     ...
     * @return process result
     */
    public StringBuilder adminBatchAddGroupResCtrlConf(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
     ...
     * @return process result
     */
    public StringBuilder adminModGroupResCtrlConf(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
     ...
     * @return process result
     */
    public StringBuilder adminBatchUpdGroupResCtrlConf(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
     ...
     * @return process result
     */
    public StringBuilder adminDelGroupResCtrlConf(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
        ...
        for (String groupName : batchGroupNames) {
            retInfo.add(
                    defMetaDataService.delGroupResCtrlConf(
                            opEntity.getModifyUser(), groupName, sBuffer, result));
        }
        return buildRetInfo(retInfo, sBuffer);
    }

    private StringBuilder innAddOrUpdGroupResCtrlConf(
        ...
        if (!WebParameterUtils.getQryPriorityIdParameter(
                ...
                false,
                (isAddOp
                        ? defClusterSetting.getQryPriorityId()
                        : TBaseConstants.META_VALUE_UNDEFINED),
                TServerConstants.QRY_PRIORITY_MIN_VALUE,
                sBuffer,
                result)) {
    ...
    private StringBuilder innBatchAddOrUpdGroupResCtrlConf(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result, boolean isAddOp) {
        // check and get operation info
        if (!WebParameterUtils.getAUDBaseInfo(req, isAddOp, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
    ...
    private boolean getGroupResCtrlJsonSetInfo(
        ...
        if (!WebParameterUtils.getJsonArrayParamValue(
                req, WebFieldDef.GROUPRESCTRLSET, true, null, result)) {
