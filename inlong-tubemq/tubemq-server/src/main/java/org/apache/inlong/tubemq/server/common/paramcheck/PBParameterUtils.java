...
public class PBParameterUtils {
     ...
     */
    public static ParamCheckResult checkProducerTopicList(
            final List<String> reqTopicLst, final StringBuilder strBuffer) {
        ParamCheckResult retResult = new ParamCheckResult();
        if (reqTopicLst == null) {
            retResult.setCheckResult(
                    false,
                ...
                if (topic.equals(TServerConstants.OFFSET_HISTORY_NAME)) {
                    retResult.setCheckResult(
                            ...
                            strBuffer
                                    ...
                                    .append(" does not allow client produce data!")
                                    .toString());
        ...
        if (transTopicList.size() > TBaseConstants.META_MAX_BOOKED_TOPIC_COUNT) {
            retResult.setCheckResult(
                    ...
                    TErrCodeConstants.BAD_REQUEST,
                    strBuffer
                            .append("Booked topic's count over max value, required max count is ")
                            .append(TBaseConstants.META_MAX_BOOKED_TOPIC_COUNT)
                            .toString());
            strBuffer.delete(0, strBuffer.length());
            return retResult;
        }
     ...
     * @return the check result
     */
    public static boolean checkConsumerTopicList(
            Set<String> depTopicSet,
        ...
        if (transTopicSet.size() > TBaseConstants.META_MAX_BOOKED_TOPIC_COUNT) {
            result.setFailResult(
                    TErrCodeConstants.BAD_REQUEST,
                    strBuff.append(
                                    "Subscribed topicList size over max value, required max count is ")
                            .append(TBaseConstants.META_MAX_BOOKED_TOPIC_COUNT)
                            .toString());
        ...
        if (!invalidTopicSet.isEmpty()) {
            result.setFailResult(
                    ...
                    strBuff.append("Requested topic [")
                            .append(invalidTopicSet)
                            .append("] not deployed!")
                            .toString());
            strBuff.delete(0, strBuff.length());
            return result.isSuccess();
     ...
     * @return the check result
     */
    public static ParamCheckResult checkConsumerOffsetSetInfo(
            ConsumeType csmType,
        ...
        for (String partOffset : partOffsetItems) {
            ...
            if (partKeyVal.length == 1) {
                retResult.setCheckResult(
                        ...
                        TErrCodeConstants.BAD_REQUEST,
                        strBuffer
                                .append("[Parameter error] unformatted Partition-Offset value : ")
                                .append(partOffset)
                                .append(" must be aa:bbb:ccc=val1,ddd:eee:ff=val2")
                                .toString());
                return retResult;
            }
            ...
            if (!reqTopicSet.contains(partKeyItems[1].trim())) {
                retResult.setCheckResult(
                        ...
                        strBuffer
                                .append(
                                        "[Parameter error] wrong offset reset for unsubscribed topic: reset item is ")
                                .append(partOffset)
                                .append(", request topicList are ")
            ...
            try {
                ...
                retResult.setCheckResult(
                        ...
                        TErrCodeConstants.BAD_REQUEST,
                        strBuffer
                                .append("[Parameter error] required long type value of ")
                                .append(partOffset)
                                .append("' Offset!")
                                .toString());
                return retResult;
    ...
    /**
     * Check the validity of consumer parameters which specify partition boostrap Offset and use
     ...
     * @return the check result
     */
    public static ParamCheckResult checkConsumerInputInfo(
        ...
        if (masterConfig.isStartOffsetResetCheck()) {
            if (offsetResetGroupEntity == null) {
                retResult.setCheckResult(
                        ...
                        TErrCodeConstants.BAD_REQUEST,
                        strBuffer
                                .append("[unauthorized subscribe] ConsumeGroup must be ")
                                .append("authorized by administrator before using bound subscribe")
                                .append(", please contact to administrator!")
                                .toString());
                strBuffer.delete(0, strBuffer.length());
                return retResult;
            }
        ...
        if (curBClientRate > allowRate) {
            ...
            retResult.setCheckResult(
                    ...
                    TErrCodeConstants.BAD_REQUEST,
                    strBuffer
                            .append("[Parameter error] System requires at least ")
                            .append(minClientCnt)
                            .append(" clients to consume data together, ")
                            .append("please add client resources!")
                            .toString());
            return retResult;
        }
        retResult.setCheckData(inConsumerInfo);
     ...
     * @return the check result
     */
    public static ParamCheckResult checkBrokerId(
            final String brokerId, final StringBuilder strBuffer) {
        ParamCheckResult retResult = new ParamCheckResult();
        if (TStringUtils.isBlank(brokerId)) {
            retResult.setCheckResult(
                    false, TErrCodeConstants.BAD_REQUEST, "Request miss necessary brokerId data");
            return retResult;
        }
        ...
        try {
            ...
            retResult.setCheckResult(
                    ...
                    TErrCodeConstants.BAD_REQUEST,
                    strBuffer
                            .append("Parse brokerId to int failure ")
                            .append(e.getMessage())
                            .toString());
            strBuffer.delete(0, strBuffer.length());
            return retResult;
     ...
     * @return the check result
     */
    public static ParamCheckResult checkClientId(
            final String clientId, final StringBuilder strBuffer) {
        return validStringParameter(
                "clientId", clientId, TBaseConstants.META_MAX_CLIENT_ID_LENGTH, strBuffer);
    }
     ...
     * @return the check result
     */
    public static ParamCheckResult checkHostName(
            final String hostName, final StringBuilder strBuffer) {
        return validStringParameter(
                "hostName", hostName, TBaseConstants.META_MAX_CLIENT_HOSTNAME_LENGTH, strBuffer);
    }
     ...
     * @return the check result
     */
    public static ParamCheckResult checkGroupName(
            final String groupName, final StringBuilder strBuffer) {
        return validStringParameter(
                "groupName", groupName, TBaseConstants.META_MAX_GROUPNAME_LENGTH, strBuffer);
    }

    private static ParamCheckResult validStringParameter(
     ...
     */
    public static boolean getStringParameter(
     ...
     * @return the check result
     */
    public static boolean getTopicNameParameter(
            String topicName,
            MetadataManager metadataManager,
            StringBuilder strBuffer,
            ProcessResult result) {
        if (!getStringParameter(WebFieldDef.TOPICNAME, topicName, strBuffer, result)) {
            return result.isSuccess();
        }
     ...
     * @return the check result
     */
    public static boolean getTopicNamePartIdInfo(
            boolean isProduce,
            String topicName,
            int partitionId,
            MetadataManager metadataManager,
            ...
            if (tgtTopicName.equals(TServerConstants.OFFSET_HISTORY_NAME)) {
                result.setFailResult(
                        ...
                        strBuffer
                                ...
                                .append(" ")
                                .append(tgtTopicName)
                                .append(" does not allow producers to send data!")
                                .toString());
                strBuffer.delete(0, strBuffer.length());
                return result.isSuccess();
            }
        ...
        if ((realPartition < 0) || (realPartition >= topicMetadata.getNumPartitions())) {
            result.setFailResult(
                    TErrCodeConstants.FORBIDDEN,
                    strBuffer
                            .append(WebFieldDef.PARTITIONID.name)
                            .append(" ")
                            .append(tgtTopicName)
                            .append("-")
                            .append(partitionId)
