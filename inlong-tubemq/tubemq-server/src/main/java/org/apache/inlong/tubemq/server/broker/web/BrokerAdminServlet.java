...
public class BrokerAdminServlet extends AbstractWebHandler {
    ...
    public void registerWebApiMethod() {
        ...
        innRegisterWebMethod("admin_query_server_version", "adminQueryBrokerVersion", false);
        ...
        innRegisterWebMethod(
                "admin_query_broker_all_consumer_info", "adminQueryBrokerAllConsumerInfo", false);
        // get memory store status info
        innRegisterWebMethod(
                "admin_query_broker_memstore_info", "adminGetMemStoreStatisInfo", false);
        // query broker's all message store info
        innRegisterWebMethod(
                "admin_query_broker_all_store_info", "adminQueryBrokerAllMessageStoreInfo", false);
        // query consumer register info
        innRegisterWebMethod(
                "admin_query_consumer_regmap", "adminQueryConsumerRegisterInfo", false);
        // manual set offset
        innRegisterWebMethod(
                "admin_manual_set_current_offset", "adminManualSetCurrentOffSet", false);
        ...
        innRegisterWebMethod(
                "admin_get_group_history_offset", "adminQueryGroupHistoryOffSet", false);
        // get broker's metric information
        innRegisterWebMethod("admin_get_metrics_info", "adminGetMetricsInfo", false);
        // get message store stats info
        innRegisterWebMethod("admin_get_msgstore_stats", "adminGetMsgStoreStatsInfo", false);
        // Enable metrics statistics
        innRegisterWebMethod("admin_enable_stats", "adminEnableMetricsStats", false);
        // Disable metrics statistics
        innRegisterWebMethod("admin_disable_stats", "adminDisableMetricsStats", false);
        // Disable unnecessary statistics
        innRegisterWebMethod("admin_disable_all_stats", "adminDisableAllStats", false);
    }
    ...
    public void adminQueryBrokerAllConsumerInfo(HttpServletRequest req, StringBuilder strBuff) {
        ...
        if (!WebParameterUtils.getStringParamValue(
                req, WebFieldDef.COMPSGROUPNAME, false, null, strBuff, result)) {
        ...
        for (Entry<String, ConsumerNodeInfo> entry : map.entrySet()) {
            ...
            Long regTime =
                    broker.getBrokerServiceServer()
                            .getConsumerRegisterTime(consumerId, entry.getKey());
            if (regTime == null || regTime <= 0) {
                strBuff.append(",\"consumerId\":\"")
                        .append(consumerId)
            ...
            strBuff.append(",\"receivedFrom\":\"")
                    .append(entry.getValue().getAddrRcvFrom())
                    .append("\",\"qryPriorityId\":")
                    .append(entry.getValue().getQryPriorityId())
                    .append(",\"curDataLimitInM\":")
                    .append(entry.getValue().getCurFlowCtrlLimitSize())
                    .append(",\"curFreqLimit\":")
                    .append(entry.getValue().getCurFlowCtrlFreqLimit())
                    .append(",\"totalSentSec\":")
                    .append(entry.getValue().getSentMsgSize())
                    .append(",\"isSupportLimit\":")
                    .append(entry.getValue().isSupportLimit())
                    .append(",\"sentUnitSec\":")
                    .append(entry.getValue().getTotalUnitSec())
                    .append(",\"totalSentMin\":")
                    .append(entry.getValue().getTotalUnitMin())
                    .append(",\"sentUnit\":")
                    .append(entry.getValue().getSentUnit());
            ...
            if (store == null) {
                ...
                strBuff.append(",\"isMessageStoreOk\":true,\"tmpOffset\":")
                        .append(tmpOffset)
                        .append(",\"minOffset\":")
                        .append(minPartOffset)
                        .append(",\"maxOffset\":")
                        .append(maxPartOffset)
                        .append(",\"zkOffset\":")
                        .append(zkOffset)
                        .append(",\"minDataOffset\":")
                        .append(minDataOffset)
                        .append(",\"maxDataOffset\":")
                        .append(maxDataOffset)
                        .append("}");
            }
        }
     ...
     */
    public void adminQueryBrokerAllMessageStoreInfo(
            HttpServletRequest req, StringBuilder sBuilder) {
        ProcessResult result = new ProcessResult();
        if (!WebParameterUtils.getStringParamValue(
                req, WebFieldDef.COMPSTOPICNAME, false, null, sBuilder, result)) {
            ...
            if (partStoreMap != null) {
                for (Entry<Integer, MessageStore> subEntry : partStoreMap.entrySet()) {
                    ...
                    sBuilder.append("{\"storeId\":")
                            ...
                            .append(numPartId)
                            .append(",\"minDataOffset\":")
                            .append(msgStore.getDataMinOffset())
                            .append(",\"maxDataOffset\":")
                            .append(msgStore.getDataMaxOffset())
                            .append(",\"sizeInBytes\":")
                            .append(msgStore.getDataStoreSize())
                            .append(",\"partitionInfo\":[");
                    for (int partitionId = 0; partitionId < numPartId; partitionId++) {
                        if (partitionId > 0) {
                            sBuilder.append(",");
                        }
                        sBuilder.append("{\"partitionId\":")
                                .append(partitionId)
                                .append(",\"minOffset\":")
                                .append(msgStore.getIndexMinOffset())
                                .append(",\"maxOffset\":")
                                .append(msgStore.getIndexMaxOffset())
                                .append(",\"sizeInBytes\":")
                                .append(msgStore.getIndexStoreSize())
                                .append("}");
                    }
                    sBuilder.append("]}");
                }
    ...
    public void adminManualSetCurrentOffSet(HttpServletRequest req, StringBuilder sBuffer) {
        ...
        if (manualOffset < store.getIndexMinOffset()) {
            sBuffer.append("{\"result\":false,\"errCode\":400,\"errMsg\":\"")
                    ...
                    .append(manualOffset)
                    .append("<")
                    .append(store.getIndexMinOffset())
                    .append(")\"}");
            return;
        }
        if (manualOffset > store.getIndexMaxOffset()) {
            sBuffer.append("{\"result\":false,\"errCode\":400,\"errMsg\":\"")
                    ...
                    .append(manualOffset)
                    .append(">")
                    .append(store.getIndexMaxOffset())
                    .append(")\"}");
            return;
        }
        OffsetService offsetService = broker.getOffsetManager();
        long oldOffset =
                offsetService.resetOffset(
                        store, groupName, topicName, partitionId, manualOffset, modifyUser);
        if (oldOffset < 0) {
     ...
     * @param sBuffer process result
     */
    public void adminQuerySnapshotMessageSet(HttpServletRequest req, StringBuilder sBuffer)
            throws Exception {
        ProcessResult result = new ProcessResult();
        if (!WebParameterUtils.getStringParamValue(
                req, WebFieldDef.TOPICNAME, true, null, sBuffer, result)) {
        ...
        final String topicName = (String) result.getRetData();
        if (!WebParameterUtils.getIntParamValue(
                req, WebFieldDef.PARTITIONID, false, -1, 0, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
        final int partitionId = (Integer) result.getRetData();
        if (!WebParameterUtils.getIntParamValue(
                req, WebFieldDef.MSGCOUNT, false, 3, 3, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
        ...
        if (!WebParameterUtils.getStringParamValue(
                req, WebFieldDef.FILTERCONDS, false, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
    ...
    public void adminQueryCurrentGroupOffSet(HttpServletRequest req, StringBuilder sBuffer) {
        ...
        if (!WebParameterUtils.getBooleanParamValue(
                req, WebFieldDef.REQUIREREALOFFSET, false, false, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
        ...
        if (requireRealOffset) {
            ...
            if (curReadDataOffset < 0) {
                sBuffer.append(",\"zkOffset\":")
                        .append(zkOffset)
                        .append(",\"curReadDataOffset\":-1,\"curRdDltDataOffset\":-1");
            } else {
                sBuffer.append(",\"zkOffset\":")
                        .append(zkOffset)
                        .append(",\"curReadDataOffset\":")
                        .append(curReadDataOffset)
                        .append(",\"curRdDltDataOffset\":")
                        .append(curRdDltDataOffset);
            }
        }
        sBuffer.append("}");
     ...
     * @param sBuffer process result
     */
    public void adminQueryConsumerRegisterInfo(HttpServletRequest req, StringBuilder sBuffer) {
        Map<String, ConsumerNodeInfo> map =
                broker.getBrokerServiceServer().getConsumerRegisterMap();
        int totalCnt = 0;
        sBuffer.append("{\"result\":true,\"errCode\":0,\"errMsg\":\"Success!\",\"dataSet\":[");
    ...
    public void adminQueryBookedGroup(HttpServletRequest req, StringBuilder sBuffer) {
        ...
        ProcessResult result = new ProcessResult();
        if (!WebParameterUtils.getBooleanParamValue(
                req, WebFieldDef.WITHDIVIDE, false, false, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
    ...
    public void adminQueryGroupOffSet(HttpServletRequest req, StringBuilder sBuffer) {
        ...
        // get group list
        if (!WebParameterUtils.getStringParamValue(
                req, WebFieldDef.COMPSGROUPNAME, false, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
    ...
    public void adminQueryGroupHistoryOffSet(HttpServletRequest req, StringBuilder sBuffer) {
        ...
        // get group list
        if (!WebParameterUtils.getStringParamValue(
                req, WebFieldDef.COMPSGROUPNAME, true, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
        ...
        if (!WebParameterUtils.getIntParamValue(
                req, WebFieldDef.MSGCOUNT, false, 1, 1, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
        ...
        if (!WebParameterUtils.getIntParamValue(
                req, WebFieldDef.MAXRETRYCOUNT, false, 2, 1, 30, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
        ...
        if (!WebParameterUtils.getDateParameter(
                req, WebFieldDef.RECORDTIME, true, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
        ...
        try {
            msgStore =
                    storeManager.getOrCreateMessageStore(TServerConstants.OFFSET_HISTORY_NAME, 0);
        } catch (Throwable ex) {
            sBuffer.append("{\"result\":false,\"errCode\":400,\"errMsg\":\"")
                    .append("Invalid parameter: not found the store by topicName(")
     ...
     */
    private void queryGroupStoredOffsets(
            MessageStore msgStore,
            String groupName,
            long requestOffset,
            long recordStamp,
    ...
    public void adminSetGroupOffSet(HttpServletRequest req, StringBuilder sBuffer) {
        ...
        // get set mode
        if (!WebParameterUtils.getBooleanParamValue(
                req, WebFieldDef.MANUALSET, true, false, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
        ...
        if (manualSet) {
            ...
            if (!WebParameterUtils.getJsonDictParamValue(
                    req, WebFieldDef.OFFSETJSON, true, null, result)) {
                WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
                return;
            }
    ...
    public void adminCloneGroupOffSet(HttpServletRequest req, StringBuilder sBuffer) {
        ...
        if (!WebParameterUtils.getStringParamValue(
                req, WebFieldDef.SRCGROUPNAME, true, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
        ...
        if (!validAndGetTopicPartInfo(
                srcGroupName, WebFieldDef.SRCGROUPNAME, srcTopicNameSet, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
        ...
        // get target consume group name
        if (!WebParameterUtils.getStringParamValue(
                req, WebFieldDef.TGTCOMPSGROUPNAME, true, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
        ...
        if (!bookedGroups.contains(srcGroupName)) {
            WebParameterUtils.buildFailResult(
                    sBuffer,
                    new StringBuilder(512)
                            .append("Parameter ")
                            .append(WebFieldDef.SRCGROUPNAME.name)
                            .append(": ")
                            .append(srcGroupName)
                            .append(" has not been registered on this Broker!")
                            .toString());
            return;
        }
    ...
    public void adminRemoveGroupOffSet(HttpServletRequest req, StringBuilder sBuffer) {
        ...
        // get set mode
        if (!WebParameterUtils.getBooleanParamValue(
                req, WebFieldDef.ONLYMEM, false, false, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
        ...
        broker.getOffsetManager().deleteGroupOffset(onlyMemory, groupTopicPartMap, modifier);
        // builder return result
    ...
    public void adminGetMetricsInfo(HttpServletRequest req, StringBuilder sBuffer) {
        ...
        if (!WebParameterUtils.getBooleanParamValue(
                req, WebFieldDef.NEEDREFRESH, false, false, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
        ...
        sBuffer.append("{\"probeTime\":\"")
                ...
                .append("\",\"nodeName\":\"")
                .append(broker.getTubeConfig().getHostName())
    ...
    public void adminEnableMetricsStats(HttpServletRequest req, StringBuilder sBuffer) {
        ProcessResult result = new ProcessResult();
        if (!WebParameterUtils.getStringParamValue(
                req, WebFieldDef.STATSTYPE, true, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return;
        }
    ...
    public void adminDisableAllStats(HttpServletRequest req, StringBuilder sBuffer) {
        innEnableOrDisableMetricsStats(false, BrokerStatsType.ALL.getName(), req, sBuffer);
    }
     ...
     * @param sBuffer query result
     */
    private void innEnableOrDisableMetricsStats(
            boolean enable, String statsType, HttpServletRequest req, StringBuilder sBuffer) {
        ProcessResult result = new ProcessResult();
    ...
    private List<Tuple3<String, Integer, Long>> buildOffsetResetInfo(Set<String> topicSet) {
        ...
        for (Map.Entry<String, Set<Integer>> entry : topicPartMap.entrySet()) {
            ...
            for (Integer partId : partitionSet) {
                ...
                result.add(new Tuple3<>(entry.getKey(), partId, store.getIndexMaxOffset()));
            }
        }
        return result;
    }
    ...
    // build reset offset info
    private boolean validManOffsetResetInfo(
            WebFieldDef fieldDef, Map<String, Long> manOffsetInfoMap, ProcessResult result) {
        String brokerId;
        String topicName;
        String strPartId;
        int partitionId;
        ...
        for (Map.Entry<String, Long> entry : manOffsetInfoMap.entrySet()) {
            ...
            if (keyItems.length != 3) {
                result.setFailResult(
                        ...
                        new StringBuilder(512)
                                ...
                                .append("'s key invalid:")
                                .append(entry.getKey())
                                .append(" must be brokerId:topicName:partitionId !")
                                .toString());
            ...
            if (!localBrokerId.equals(brokerId) || !topicConfigMap.containsKey(topicName)) {
                continue;
            }
            try {
                ...
                result.setFailResult(
                        ...
                        new StringBuilder(512)
                                ...
                                .append("'s key invalid:")
                                .append(entry.getKey())
                                .append("'s partitionId value not number!")
                                .toString());
                return result.isSuccess();
            }
            ...
            adjOffset =
                    MixedUtils.mid(
                            entry.getValue(), store.getIndexMinOffset(), store.getIndexMaxOffset());
    ...
    private boolean validAndGetGroupTopicInfo(
            Set<String> groupSet, Set<String> topicSet, ProcessResult result) {
    ...
    private boolean validAndGetTopicPartInfo(
            String groupName, WebFieldDef groupFldDef, Set<String> topicSet, ProcessResult result) {
        ...
        if (subTopicSet == null || subTopicSet.isEmpty()) {
            result.setFailResult(
                    ...
                    new StringBuilder(512)
                            .append("Parameter ")
                            .append(groupFldDef.name)
                            .append(": subscribed topic set of ")
                            .append(groupName)
                            .append(" query result is null!")
                            .toString());
            return result.isSuccess();
        }
            ...
            if (tgtTopicSet.isEmpty()) {
                result.setFailResult(
                        ...
                        new StringBuilder(512)
                                ...
                                .append(": ")
                                .append(groupName)
                                .append(" unsubscribed to the specified topic set!")
                                .toString());
                return result.isSuccess();
            }
        }
        ...
        if (topicPartMap.isEmpty()) {
            result.setFailResult(
                    ...
                    new StringBuilder(512)
                            .append("Parameter ")
                            .append(groupFldDef.name)
                            .append(": all topics subscribed by the group have been deleted!")
                            .toString());
            return result.isSuccess();
        }
