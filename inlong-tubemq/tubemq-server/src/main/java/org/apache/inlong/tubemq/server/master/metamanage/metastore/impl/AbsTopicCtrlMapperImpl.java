...
public abstract class AbsTopicCtrlMapperImpl implements TopicCtrlMapper {
    ...
    // data cache
    private final ConcurrentHashMap<String /* topicName */, TopicCtrlEntity> topicCtrlCache =
    ...
    @Override
    public boolean addTopicCtrlConf(
    ...
    @Override
    public boolean updTopicCtrlConf(
            TopicCtrlEntity entity, StringBuilder strBuff, ProcessResult result) {
        // Checks whether the record already exists
        TopicCtrlEntity curEntity = topicCtrlCache.get(entity.getTopicName());
        if (curEntity == null) {
        ...
        if (!newEntity.updModifyInfo(
            ...
            result.setFailResult(
                    DataOpErrCode.DERR_UNCHANGED.getCode(), "Topic control configure not changed!");
            return result.isSuccess();
        }
        // Store data to persistent
