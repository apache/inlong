...
public class ZKUtil {
    ...
    /**
     * Join the prefix zkNode name with the suffix zkNode name to generate a proper
     * full zkNode name.
     * <p/>
     * Assumes prefix does not end with slash and suffix does not begin with it.
     *
     * @param prefix
     *          beginning of zkNode name
     * @param suffix
     *          ending of zkNode name
     ...
     * <p/>
     * Returns the children data. Returns null if the node does not exist or there
     * is an exception.
     *
     * @param zkw
     *          zk reference
    ...
    /**
     * Sets the data of the existing zkNode to be the specified data. Ensures that
     ...
     *           if unexpected zookeeper exception
     */
    public static boolean setData(ZooKeeperWatcher zkw, String zkNode, byte[] data,
            int expectedVersion)
    ...
    /**
     * Sets the data of the existing zkNode to be the specified data. The node must
     ...
     */
    public static int createAndWatch(ZooKeeperWatcher zkw, String zkNode,
            byte[] data)
    ...
    /**
     * Creates the specified node, if the node does not exist. Does not set a watch
     * and fails silently if the node already exists.
     * <p/>
     * The node created is persistent and open access.
     *
     * @param zkw
     ...
     *          {@link NodeExistsException} is thrown in case the path already
     *          exists
     ...
     */
    public static void createPersistent(ZooKeeperWatcher zkw, String path,
     ...
     */
    public static void createParentPath(final ZooKeeperWatcher zkw, final String path) throws Exception {
    ...
    /**
     * Update the value of a persistent node with the given path and data. create
     * parent directory if necessary. Never throw NodeExistException.
     */
    ...
    public static String readData(final ZooKeeperWatcher zkw, final String path) throws KeeperException {
    ...
    public static String readDataMaybeNull(final ZooKeeperWatcher zkw, final String path) throws KeeperException {
        try {
    ...
    /**
     * Normalize ZooKeeper path string. The ZooKeeper path must start with "/" and
     * not end with "/"
     *
     * @param root
     ...
     * of children of the specified node. If the node exists but it has no children,
     ...
     * @return list of data of children of the specified node, an empty list if the
     *         node exists but has no children, and null if the node does not exist
     * @throws KeeperException
     *           if unexpected zookeeper exception
     */
     ...
     * specified zNode which will capture a NodeDeleted event on the specified zNode
     * as well as NodeChildrenChanged if any children of the specified zNode are
     * created or deleted.
     *
     * Returns null if the specified node does not exist. Otherwise returns a list
     * of children of the specified node. If the node exists but it has no children,
    ...
    /**
     * Watch the specified zNode for delete/create/change events. The watcher is set
     * whether or not the node exists. If the node already exists, the method
