 ...
 *
 * <p>Methods all throw {@link KeeperException} if there is an unexpected zookeeper exception, so
...
public class ZKUtil {
     ...
     *
     * <p>Sets the connection status monitoring watcher to the specified watcher.
     *
     * <p>configuration to pull ensemble and other settings from
     *
     * @param watcher watcher to monitor connection changes
     * @return connection to zookeeper
     * @throws IOException if unable to connect to zk or config problem
     */
    public static RecoverableZooKeeper connect(ZKConfig zkConfig, Watcher watcher)
            throws IOException {
        if (zkConfig.getZkServerAddr() == null) {
            throw new IOException("Unable to determine ZooKeeper Server Address String");
        }
        return new RecoverableZooKeeper(
                zkConfig.getZkServerAddr(),
    ...
    /**
     * Join the prefix zkNode name with the suffix zkNode name to generate a proper full zkNode
     * name.
     *
     * <p>Assumes prefix does not end with slash and suffix does not begin with it.
     *
     ...
     *
     * <p>Returns true if node exists, false if not. Returns an exception if there is an unexpected
     * zookeeper exception.
     *
     ...
     *
     * <p>Returns the data and sets a watch if the node exists. Returns null and no watch is set if
     ...
     * @throws KeeperException if unexpected zookeeper exception
     */
    public static byte[] getDataAndWatch(ZooKeeperWatcher zkw, String zkNode)
     ...
     *
     * <p>Returns the children data. Returns null if the node does not exist or there is an
     * exception.
     *
     * @param zkw zk reference
     * @param zkNode path of node
    ...
    private static byte[] getDataInternal(
            ZooKeeperWatcher zkw, String zkNode, Stat stat, boolean watcherSet)
            throws KeeperException {
        try {
     ...
     * <p>
     *
     * <p>No watches are set but setting data will trigger other watchers of this node.
     *
     * <p>
     *
    ...
    public static boolean setData(
            ZooKeeperWatcher zkw, String zkNode, byte[] data, int expectedVersion)
     ...
     * @param data data to set for node
     */
    public static void createSetData(
            final ZooKeeperWatcher zkw, final String zkNode, final byte[] data)
            throws KeeperException {
    ...
    public static int createAndWatch(ZooKeeperWatcher zkw, String zkNode, byte[] data)
        ...
        try {
            ...
            zkw.getRecoverableZooKeeper()
                    .create(zkNode, data, createACL(zkw, zkNode), CreateMode.PERSISTENT);
     ...
     * @throws KeeperException if unexpected zookeeper exception
     */
    public static void createAndFailSilent(ZooKeeperWatcher zkw, String zkNode)
            throws KeeperException {
        try {
     ...
     *
     * <p>No watches are set and no errors are thrown if the node already exists.
     *
     * <p>The nodes created are persistent and open access.
     *
     * @param zkw zk reference
     * @param zkNode path of node
     * @throws KeeperException if unexpected zookeeper exception
     */
    public static void createWithParents(ZooKeeperWatcher zkw, String zkNode)
            throws KeeperException {
        try {
            if (zkNode == null) {
                return;
            ...
            waitForZKConnectionIfAuthenticating(zkw);
            zkw.getRecoverableZooKeeper()
                    .create(zkNode, new byte[0], createACL(zkw, zkNode), CreateMode.PERSISTENT);
        } catch (KeeperException.NodeExistsException nee) {
            return;
        ...
        logger.debug(
                zkw.prefix(
                        "Retrieved "
                                ...
                                + " byte(s) of data from zkNode "
                                + zkNode
                                + (watcherSet ? " and set watcher; " : "; data=")
                                + (data == null
                                        ? "null"
                                        : data.length == 0 ? "empty" : new String(data))));
    }
     ...
     */
    public static void updatePersistentPath(
            final ZooKeeperWatcher zkw, final String path, final String data) throws Exception {
    ...
    /**
     * Normalize ZooKeeper path string. The ZooKeeper path must start with "/" and not end with "/"
     *
     ...
     * NodeChildrenChanged if any children of the specified zNode are created or deleted.
     *
     * <p>Returns null if the specified node does not exist. Otherwise returns a list of children of
     * the specified node. If the node exists but it has no children, an empty list will be
     * returned.
     *
     * @param zkw zk reference
     ...
     * which will capture a NodeDeleted event on the specified zNode as well as NodeChildrenChanged
     * if any children of the specified zNode are created or deleted.
     *
     * <p>Returns null if the specified node does not exist. Otherwise returns a list of children of
     * the specified node. If the node exists but it has no children, an empty list will be
     * returned.
     ...
     *
     * <p>If the node is created successfully, a watcher is also set on the node.
     *
     * <p>If the node is not created successfully because it already exists, this method will also
     * set a watcher on the node.
     *
     * <p>If there is another problem, a KeeperException will be thrown.
     *
     * @param zkw zk reference
     ...
     * @throws KeeperException if unexpected zookeeper exception
     */
    public static boolean createEphemeralNodeAndWatch(
            ZooKeeperWatcher zkw, String zNode, byte[] data) throws KeeperException {
     ...
     */
    public static boolean createEphemeralNodeAndWatch(
            ZooKeeperWatcher zkw, String zNode, byte[] data, CreateMode mode)
            throws KeeperException {
        try {
            waitForZKConnectionIfAuthenticating(zkw);
            zkw.getRecoverableZooKeeper().create(zNode, data, createACL(zkw, zNode), mode);
        } catch (KeeperException.NodeExistsException nee) {
