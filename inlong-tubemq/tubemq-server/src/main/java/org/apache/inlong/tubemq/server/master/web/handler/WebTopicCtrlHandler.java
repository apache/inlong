...
public class WebTopicCtrlHandler extends AbstractWebHandler {
    ...
    public void registerWebApiMethod() {
        ...
        registerModifyWebMethod(
                "admin_batch_update_topic_control_info", "adminBatchModTopicCtrlInfo");
     ...
     */
    public StringBuilder adminQueryTopicCtrlInfo(
     ...
     * @return process result
     */
    public StringBuilder adminAddTopicCtrlInfo(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
     ...
     * @return process result
     */
    public StringBuilder adminBatchAddTopicCtrlInfo(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
     ...
     * @return process result
     */
    public StringBuilder adminModTopicCtrlInfo(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
     ...
     * @return process result
     */
    public StringBuilder adminBatchModTopicCtrlInfo(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
     ...
     * @return process result
     */
    public StringBuilder adminDeleteTopicCtrlInfo(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
        ...
        for (String topicName : topicNameSet) {
            defMetaDataService.delTopicCtrlConf(
                    opEntity.getModifyUser(), topicName, sBuffer, result);
            retInfo.add(
                    new TopicProcessResult(TBaseConstants.META_VALUE_UNDEFINED, topicName, result));
        }
        return buildRetInfo(retInfo, sBuffer);
    }

    private StringBuilder innAddOrUpdTopicCtrlInfo(
        ...
        if (topicNameSet.size() == 1) {
            if (!WebParameterUtils.getIntParamValue(
                    req,
                    WebFieldDef.TOPICNAMEID,
                    false,
                    (isAddOp ? TServerConstants.TOPIC_ID_MIN : TBaseConstants.META_VALUE_UNDEFINED),
                    TServerConstants.TOPIC_ID_MIN,
                    sBuffer,
                    result)) {
                WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
                return sBuffer;
        ...
        for (String topicName : topicNameSet) {
            retInfo.add(
                    defMetaDataService.addOrUpdTopicCtrlConf(
                            isAddOp,
                            opEntity,
                            topicName,
                            topicNameId,
    ...
    private StringBuilder innBatchAddOrUpdTopicCtrlInfo(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result, boolean isAddOp) {
        // check and get operation info
        if (!WebParameterUtils.getAUDBaseInfo(req, isAddOp, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
    ...
    private boolean getTopicCtrlJsonSetInfo(
        ...
        if (!WebParameterUtils.getJsonArrayParamValue(
                req, WebFieldDef.TOPICCTRLSET, true, null, result)) {
        ...
        for (Map<String, String> itemConfMap : ctrlJsonArray) {
            ...
            if (!WebParameterUtils.getAUDBaseInfo(
                    itemConfMap, isAddOp, defOpEntity, sBuffer, result)) {
                return result.isSuccess();
            }
            final BaseEntity itemOpEntity = (BaseEntity) result.getRetData();
            // get topicName configure info
            if (!WebParameterUtils.getStringParamValue(
                    itemConfMap, WebFieldDef.TOPICNAME, true, "", sBuffer, result)) {
                return result.isSuccess();
            }
