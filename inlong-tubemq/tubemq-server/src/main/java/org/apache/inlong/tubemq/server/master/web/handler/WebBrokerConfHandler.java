...
/**
 * The class to handle the default config of broker, including: - Add config - Update config -
 * Delete config And manage the broker status.
 *
 * <p>Please note that one IP could only host one broker, and brokerId must be unique
 */
public class WebBrokerConfHandler extends AbstractWebHandler {
    ...
    public void registerWebApiMethod() {
        ...
        registerModifyWebMethod(
                "admin_batch_update_broker_configure", "adminBatchUpdBrokerConfInfo");
        ...
        registerModifyWebMethod(
                "admin_release_broker_autoforbidden_status", "adminRelBrokerAutoForbiddenStatus");
     ...
     */
    public StringBuilder adminQueryBrokerConfInfo(
        ...
        if (!WebParameterUtils.getStringParamValue(
                req, WebFieldDef.COMPBROKERIP, false, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return sBuffer;
        }
        ...
        if (!WebParameterUtils.getIntParamValue(
                req,
                WebFieldDef.REGIONID,
                false,
                TBaseConstants.META_VALUE_UNDEFINED,
                TServerConstants.BROKER_REGION_ID_MIN,
                sBuffer,
                result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return sBuffer;
        ...
        if (!WebParameterUtils.getIntParamValue(
                ...
                false,
                TBaseConstants.META_VALUE_UNDEFINED,
                TServerConstants.BROKER_GROUP_ID_MIN,
                sBuffer,
                result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return sBuffer;
        ...
        if (!WebParameterUtils.getBooleanParamValue(
                req, WebFieldDef.ISINCLUDE, false, true, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return sBuffer;
        }
        ...
        // get withTopic info
        if (!WebParameterUtils.getBooleanParamValue(
                req, WebFieldDef.WITHTOPIC, false, false, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return sBuffer;
        }
     ...
     * @return process result
     */
    public StringBuilder adminAddBrokerConfInfo(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
     ...
     * @return process result
     */
    public StringBuilder adminUpdateBrokerConfInfo(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
     ...
     * @return process result
     */
    public StringBuilder adminBatchAddBrokerConfInfo(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
     ...
     * @return process result
     */
    public StringBuilder adminBatchUpdBrokerConfInfo(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
     ...
     * @return process result
     */
    public StringBuilder adminDeleteBrokerConfEntityInfo(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
        ...
        if (!WebParameterUtils.getBooleanParamValue(
                req, WebFieldDef.ISRESERVEDDATA, false, false, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return sBuffer;
        }
        ...
        for (Integer brokerId : brokerIds) {
            defMetaDataService.delBrokerConfInfo(
                    opEntity.getModifyUser(), isReservedData, brokerId, sBuffer, result);
    ...
    public StringBuilder adminOnlineBrokerConf(
        ...
        for (Integer brokerId : brokerIds) {
            retInfo.add(
                    defMetaDataService.changeBrokerConfStatus(
                            opEntity,
                            brokerId,
                            ManageStatus.STATUS_MANAGE_ONLINE,
                            sBuffer,
                            result));
        }
        return buildRetInfo(retInfo, sBuffer);
     ...
     * @return process result
     */
    public StringBuilder adminOfflineBrokerConf(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
        // check and get operation info
        if (!WebParameterUtils.getAUDBaseInfo(req, false, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
        ...
        for (Integer brokerId : brokerIds) {
            retInfo.add(
                    defMetaDataService.changeBrokerConfStatus(
                            opEntity,
                            brokerId,
                            ManageStatus.STATUS_MANAGE_OFFLINE,
                            sBuffer,
                            result));
        }
        return buildRetInfo(retInfo, sBuffer);
    ...
    /**
     * Set read/write status of a broker. The same operations could be made by changing broker's
     * config, but those are extracted here to simplify the code.
     *
     * @param req Http Servlet Request
     * @param sBuffer string buffer
     * @param result process result
     * @return process result
     */
    public StringBuilder adminSetReadOrWriteBrokerConf(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
        // check and get operation info
        if (!WebParameterUtils.getAUDBaseInfo(req, false, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
        ...
        Tuple2<Boolean, Boolean> rdWtTpl = (Tuple2<Boolean, Boolean>) result.getRetData();
        ...
        for (Integer brokerId : brokerIds) {
            ...
            retInfo.add(
                    defMetaDataService.changeBrokerConfStatus(
                            opEntity, brokerId, newMngStatus, sBuffer, result));
        }
        return buildRetInfo(retInfo, sBuffer);
    }
     ...
     * @return process result
     */
    public StringBuilder adminReloadBrokerConf(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result) {
        // check and get operation info
        if (!WebParameterUtils.getAUDBaseInfo(req, false, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
        ...
        for (Integer brokerId : brokerIds) {
            retInfo.add(
                    defMetaDataService.reloadBrokerConfInfo(opEntity, brokerId, sBuffer, result));
        }
        return buildRetInfo(retInfo, sBuffer);
    }
    ...
    public StringBuilder adminRelBrokerAutoForbiddenStatus(
        ...
        if (!WebParameterUtils.getStringParamValue(
                req, WebFieldDef.RELREASON, false, "Web API call", sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return sBuffer;
        }
    ...
    public StringBuilder adminQueryBrokerRunStatusInfo(
        ...
        // get withDetail info
        if (!WebParameterUtils.getBooleanParamValue(
                req, WebFieldDef.WITHDETAIL, false, false, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return sBuffer;
        }
        ...
        if (!WebParameterUtils.getBooleanParamValue(
                req, WebFieldDef.ONLYABNORMAL, false, false, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return sBuffer;
        }
        ...
        if (!WebParameterUtils.getBooleanParamValue(
                req, WebFieldDef.ONLYAUTOFBD, false, false, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return sBuffer;
        }
        ...
        if (!WebParameterUtils.getBooleanParamValue(
                req, WebFieldDef.ONLYENABLETLS, false, false, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return sBuffer;
        }
        ...
        for (BrokerConfEntity entity : brokerConfEntityMap.values()) {
            ...
            sBuffer.append("{\"brokerId\":")
                    ...
                    .append("\",\"brokerPort\":")
                    .append(entity.getBrokerPort())
                    .append(",\"brokerWebPort\":")
                    .append(entity.getBrokerWebPort())
                    .append(",\"manageStatus\":\"")
                    .append(entity.getManageStatusStr())
                    .append("\"");
            if (brokerInfo == null) {
                sBuffer.append(",\"brokerTLSPort\":")
                        .append(entity.getBrokerTLSPort())
                        .append(",\"enableTLS\":\"-\"");
            } else {
                sBuffer.append(",\"brokerTLSPort\":")
                        .append(entity.getBrokerTLSPort())
            ...
            if (entity.getManageStatus() == ManageStatus.STATUS_MANAGE_APPLY) {
                sBuffer.append(",\"runStatus\":\"-\",\"subStatus\":\"-\"")
                        .append(
                                ",\"isConfChanged\":\"-\",\"isConfLoaded\":\"-\",\"isBrokerOnline\":\"-\"")
                        .append(
                                ",\"brokerVersion\":\"-\",\"acceptPublish\":\"-\",\"acceptSubscribe\":\"-\"");
            } else {
                ...
                if (runStatusInfo == null) {
                    ...
                    sBuffer.append(",\"isConfChanged\":\"")
                            .append(syncTuple.getF0())
                            .append("\",\"isConfLoaded\":\"")
                            .append(syncTuple.getF1())
                            .append("\",\"isBrokerOnline\":\"")
                            .append(runStatusInfo.isOnline())
                            .append("\"")
                            .append(",\"brokerVersion\":\"-\",\"acceptPublish\":\"")
                            .append(pubSubTuple.getF0())
                            .append("\",\"acceptSubscribe\":\"")
                            .append(pubSubTuple.getF1())
                            .append("\"");
                    if (withDetail) {
    ...
    private StringBuilder innAddOrUpdBrokerConfInfo(
        ...
        if (!WebParameterUtils.getIntParamValue(
                ...
                WebFieldDef.BROKERPORT,
                false,
                (isAddOp ? defClusterSetting.getBrokerPort() : TBaseConstants.META_VALUE_UNDEFINED),
                1,
                sBuffer,
                result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
        ...
        if (!WebParameterUtils.getIntParamValue(
                ...
                false,
                (isAddOp
                        ? defClusterSetting.getBrokerTLSPort()
                        : TBaseConstants.META_VALUE_UNDEFINED),
                1,
                sBuffer,
                result)) {
        ...
        if (!WebParameterUtils.getIntParamValue(
                ...
                false,
                (isAddOp
                        ? defClusterSetting.getBrokerWebPort()
                        : TBaseConstants.META_VALUE_UNDEFINED),
                1,
                sBuffer,
                result)) {
        ...
        if (!WebParameterUtils.getIntParamValue(
                ...
                false,
                (isAddOp
                        ? TServerConstants.BROKER_REGION_ID_DEF
                        : TBaseConstants.META_VALUE_UNDEFINED),
                TServerConstants.BROKER_REGION_ID_MIN,
                sBuffer,
                result)) {
        ...
        if (!WebParameterUtils.getIntParamValue(
                ...
                false,
                (isAddOp
                        ? TServerConstants.BROKER_GROUP_ID_DEF
                        : TBaseConstants.META_VALUE_UNDEFINED),
                TServerConstants.BROKER_GROUP_ID_MIN,
                sBuffer,
                result)) {
        ...
        if (!WebParameterUtils.getTopicPropInfo(
                req, (isAddOp ? defClusterSetting.getClsDefTopicProps() : null), sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
            return sBuffer;
        }
        ...
        if (isAddOp) {
            ...
            retInfo.add(
                    defMetaDataService.addOrUpdBrokerConfig(
                            isAddOp,
                            opEntity,
                            brokerIdAndIpTuple.getF0(),
                            brokerIdAndIpTuple.getF1(),
                            brokerPort,
                            brokerTlsPort,
                            brokerWebPort,
                            regionId,
     ...
     * @return
     */
    private StringBuilder innBatchAddOrUpdBrokerConfInfo(
            HttpServletRequest req, StringBuilder sBuffer, ProcessResult result, boolean isAddOp) {
        // check and get operation info
        if (!WebParameterUtils.getAUDBaseInfo(req, isAddOp, null, sBuffer, result)) {
            WebParameterUtils.buildFailResult(sBuffer, result.getErrMsg());
     ...
     * @return
     */
    private StringBuilder addTopicInfo(
            Boolean withTopic,
    ...
    private boolean getBrokerJsonSetInfo(
        ...
        if (!WebParameterUtils.getJsonArrayParamValue(
                req, WebFieldDef.BROKERJSONSET, true, null, result)) {
            return result.isSuccess();
        }
        List<Map<String, String>> brokerJsonArray = (List<Map<String, String>>) result.getRetData();
        // check and get cluster default setting info
        ClusterSettingEntity defClusterSetting = defMetaDataService.getClusterDefSetting(false);
        ...
        for (Map<String, String> brokerObject : brokerJsonArray) {
            ...
            if (!WebParameterUtils.getAUDBaseInfo(
                    brokerObject, isAddOp, defOpEntity, sBuffer, result)) {
                return result.isSuccess();
            }
            ...
            if (!WebParameterUtils.getIntParamValue(
                    ...
                    false,
                    (isAddOp
                            ? defClusterSetting.getBrokerPort()
                            : TBaseConstants.META_VALUE_UNDEFINED),
                    1,
                    sBuffer,
                    result)) {
            ...
            if (!WebParameterUtils.getTopicPropInfo(
                    brokerObject,
                    (isAddOp ? defClusterSetting.getClsDefTopicProps() : null),
                    sBuffer,
                    result)) {
                return result.isSuccess();
            }
            ...
            if (isAddOp) {
                ...
                // check and get brokerId field
                if (!WebParameterUtils.getIntParamValue(
                        brokerObject, WebFieldDef.BROKERID, true, sBuffer, result)) {
                    return result.isSuccess();
                }
    ...
    private <T> boolean getBrokerIpAndIdParamValue(
            T paramCntr, StringBuilder sBuffer, ProcessResult result) {
        // get brokerIp
        if (!WebParameterUtils.getStringParamValue(
                paramCntr, WebFieldDef.BROKERIP, true, null, sBuffer, result)) {
            return result.isSuccess();
        }
        String brokerIp = (String) result.getRetData();
        // get brokerId
        if (!WebParameterUtils.getIntParamValue(
                paramCntr, WebFieldDef.BROKERID, false, 0, 0, sBuffer, result)) {
            return result.isSuccess();
        }
    ...
    private <T> boolean getManageStatusParamValue(
            T paramCntr, StringBuilder sBuffer, ProcessResult result) {
        if (!WebParameterUtils.getStringParamValue(
                paramCntr, WebFieldDef.MANAGESTATUS, false, null, sBuffer, result)) {
            return result.isSuccess();
        }
    ...
    private <T> boolean getAcceptReadAndWriteParamValue(
            T paramCntr, StringBuilder sBuffer, ProcessResult result) {
        if (!WebParameterUtils.getBooleanParamValue(
                paramCntr, WebFieldDef.ACCEPTPUBLISH, false, null, sBuffer, result)) {
            return result.isSuccess();
        }
        Boolean publishParam = (Boolean) result.getRetData();
        if (!WebParameterUtils.getBooleanParamValue(
                paramCntr, WebFieldDef.ACCEPTSUBSCRIBE, false, null, sBuffer, result)) {
            return result.isSuccess();
        }
        ...
        if (publishParam == null && subscribeParam == null) {
            result.setFailResult(
                    ...
                    sBuffer.append("Fields ")
                            .append(WebFieldDef.ACCEPTPUBLISH.name)
                            .append(" or ")
                            .append(WebFieldDef.ACCEPTSUBSCRIBE.name)
                            .append(" must exist at this method!")
                            .toString());
            sBuffer.delete(0, sBuffer.length());
            return result.isSuccess();
        }
