...
public class BrokerConfEntity extends BaseEntity implements Cloneable {
    ...
    public BrokerConfEntity(
            BaseEntity opEntity, int brokerId, String brokerIp, ClusterSettingEntity defSetting) {
        super(opEntity);
        this.brokerWebPort = defSetting.getBrokerWebPort();
        this.topicProps.updModifyInfo(defSetting.getClsDefTopicProps());
        setBrokerIpAndAllPort(
                brokerId, brokerIp, defSetting.getBrokerPort(), defSetting.getBrokerTLSPort());
    }
    ...
    public void setBrokerIpAndAllPort(
            int brokerId, String brokerIp, int brokerPort, int brokerTLSPort) {
        this.brokerId = brokerId;
        this.brokerIp = brokerIp;
    ...
    public boolean updModifyInfo(
        ...
        if (regionId != TBaseConstants.META_VALUE_UNDEFINED && this.regionId != regionId) {
            changed = true;
            this.regionId = regionId;
        }
        // check and set regionId info
        if (groupId != TBaseConstants.META_VALUE_UNDEFINED && this.groupId != groupId) {
            changed = true;
            this.groupId = groupId;
        }
    ...
    /**
     * Check whether the specified query item value matches Allowed query items: brokerId, brokerIp,
     * brokerPort, brokerTLSPort, regionId, groupId manageStatus, brokerWebPort
     *
    ...
    public StringBuilder toWebJsonStr(
        ...
        if (isLongName) {
            sBuffer.append("{\"brokerId\":")
                    ...
                    .append(brokerWebPort)
                    .append(",\"manageStatus\":\"")
                    .append(manageStatus.getDescription())
                    .append("\"")
    ...
    public void getBrokerDefaultConfInfo(ClusterSettingEntity defSetting, StringBuilder strBuff) {
        ...
        strBuff.append(
                        (topicProps.getNumPartitions() == TBaseConstants.META_VALUE_UNDEFINED)
                                ? defTopicProps.getNumPartitions()
                                : topicProps.getNumPartitions())
                .append(TokenConstants.ATTR_SEP)
                .append(
                        (topicProps.getAcceptPublish() == null)
                                ? defTopicProps.isAcceptPublish()
                                : topicProps.isAcceptPublish())
                .append(TokenConstants.ATTR_SEP)
                .append(
                        (topicProps.getAcceptSubscribe() == null)
                                ? defTopicProps.isAcceptSubscribe()
                                : topicProps.isAcceptSubscribe())
                .append(TokenConstants.ATTR_SEP)
                .append(
                        (topicProps.getUnflushThreshold() == TBaseConstants.META_VALUE_UNDEFINED)
                                ? defTopicProps.getUnflushThreshold()
                                : topicProps.getUnflushThreshold())
                .append(TokenConstants.ATTR_SEP)
                .append(
                        (topicProps.getUnflushInterval() == TBaseConstants.META_VALUE_UNDEFINED)
                ...
                .append(TokenConstants.ATTR_SEP)
                .append(
                        (TStringUtils.isEmpty(topicProps.getDeletePolicy()))
                                ? defTopicProps.getDeletePolicy()
                                : topicProps.getDeletePolicy())
                .append(TokenConstants.ATTR_SEP)
                .append(
                        (topicProps.getNumTopicStores() == TBaseConstants.META_VALUE_UNDEFINED)
                                ? defTopicProps.getNumTopicStores()
                                : topicProps.getNumTopicStores())
                .append(TokenConstants.ATTR_SEP)
                .append(
                        (topicProps.getUnflushDataHold() == TBaseConstants.META_VALUE_UNDEFINED)
                                ? defTopicProps.getUnflushDataHold()
                                : topicProps.getUnflushDataHold())
                .append(TokenConstants.ATTR_SEP)
                .append(
                        (topicProps.getMemCacheMsgSizeInMB() == TBaseConstants.META_VALUE_UNDEFINED)
                                ? defTopicProps.getMemCacheMsgSizeInMB()
                                : topicProps.getMemCacheMsgSizeInMB())
                .append(TokenConstants.ATTR_SEP)
                .append(
                        (topicProps.getMemCacheMsgCntInK() == TBaseConstants.META_VALUE_UNDEFINED)
                                ? defTopicProps.getMemCacheMsgCntInK()
                                : topicProps.getMemCacheMsgCntInK())
                .append(TokenConstants.ATTR_SEP)
                .append(
                        (topicProps.getMemCacheFlushIntvl() == TBaseConstants.META_VALUE_UNDEFINED)
