        ...
        if (!config.validateAndRecord(var10001, var10002::error)) {
            ...
            if (this.running.get()) {
                throw new IllegalStateException(
                        "Database history file already initialized to " + this.path);
            } else {
        ...
        this.lock.write(
                () -> {
                    if (this.running.compareAndSet(false, true)) {
                        ...
                        if (path == null) {
                            throw new IllegalStateException(
                                    "FileDatabaseHistory must be configured before it is started");
                        }
                        ...
                        try {
                            ...
                            throw new DatabaseHistoryException(
                                    "Unable to create history file at "
                                            + path
                                            + ": "
                                            + var4.getMessage(),
                                    var4);
                    ...
                    () -> {
                        if (!this.running.get()) {
                            ...
                            try {
                                ...
                                try {
                                    BufferedWriter historyWriter =
                                            Files.newBufferedWriter(
                                                    this.path, StandardOpenOption.APPEND);
                                    ...
                                    label58:
                                    {
                                        try {
                                            try {
                                                ...
                                                this.logger.error(
                                                        "Failed to add record to history at {}: {}",
                                ...
                                this.logger.error(
                                        "Failed to convert record to string: {}", record, var10);
                            }
                        }
                    });
        }
        ...
        this.lock.write(
                () -> {
                    try {
                        ...
                        this.logger.error(
                                "Failed to add recover records from history at {}",
                                this.path,
                                var4);
                    }
                });
