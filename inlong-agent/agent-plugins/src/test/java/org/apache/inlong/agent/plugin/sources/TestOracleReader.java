...
@PrepareForTest({AgentDbUtils.class, MetricRegister.class, AuditUtils.class, OracleReader.class})
...
public class TestOracleReader {
    ...
    public void setUp() throws Exception {
        ...
        when(jobProfile.get(eq(OracleReader.JOB_DATABASE_SERVICE_NAME))).thenReturn(dbname);
        when(jobProfile.get(eq(OracleReader.JOB_DATABASE_DRIVER_CLASS), anyString())).thenReturn(
                OracleReader.DEFAULT_JOB_DATABASE_DRIVER_CLASS);
        when(jobProfile.getInt(eq(OracleReader.JOB_DATABASE_BATCH_SIZE), anyInt())).thenReturn(
                OracleReader.DEFAULT_JOB_DATABASE_BATCH_SIZE);
        when(jobProfile.get(eq(OracleReader.JOB_DATABASE_TYPE), anyString())).thenReturn(
                OracleReader.ORACLE);
        when(jobProfile.get(eq(OracleReader.JOB_DATABASE_SEPARATOR), anyString())).thenReturn(
                OracleReader.STD_FIELD_SEPARATOR_SHORT);
        mockStatic(AgentDbUtils.class);
        when(AgentDbUtils.getConnectionFailover(eq(OracleReader.DEFAULT_JOB_DATABASE_DRIVER_CLASS), anyString(),
        ...
        (reader = new OracleReader(sql)).init(jobProfile);
    }
    ...
    public void testRead() throws Exception {
        ...
        final String msg1 = String.join(OracleReader.STD_FIELD_SEPARATOR_SHORT, v11, v12);
        final String msg2 = String.join(OracleReader.STD_FIELD_SEPARATOR_SHORT, v21, v22);
        ...
        verify(preparedStatement, times(1)).setFetchSize(OracleReader.DEFAULT_JOB_DATABASE_BATCH_SIZE);
