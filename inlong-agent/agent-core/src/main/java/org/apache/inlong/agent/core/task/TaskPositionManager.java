...
/**
 * used to store task position to db, task position is stored as properties in JobProfile. where key
 * is task read file name and value is task sink position note that this class is generated
 */
public class TaskPositionManager extends AbstractDaemon {
    ...
    public static TaskPositionManager getTaskPositionManager() {
        if (taskPositionManager == null) {
            throw new RuntimeException(
                    "task position manager has not been initialized by agentManager");
        }
        return taskPositionManager;
    }
            ...
            while (isRunnable()) {
                try {
                    ...
                    for (String jobId : jobTaskPositionMap.keySet()) {
                        ...
                        if (jobProfile == null) {
                            LOGGER.warn(
                                    "jobProfile {} cannot be found in db, "
                                            + "might be deleted by standalone mode, now delete job position in memory",
                                    jobId);
                    ...
                    int flushTime =
                            conf.getInt(AGENT_HEARTBEAT_INTERVAL, DEFAULT_AGENT_FETCHER_INTERVAL);
    ...
    private void flushJobProfile(String jobId, JobProfile jobProfile) {
        ...
        if (jobConfDb.checkJobfinished(jobProfile)) {
            LOGGER.info(
                    "Cannot update job profile {}, delete memory job in jobTaskPosition", jobId);
            deleteJobPosition(jobId);
        } else {
