...
public class Constants {
    ...
    /**
     * It uses dt as the built-in data time field name. It's a work-around solution and should be
     * replaced by later.
     */
    ...
    public static final ConfigOption<String> ZOOKEEPER_QUORUM =
            key("zookeeper.quorum").noDefaultValue().withDescription("The ZooKeeper quorum to use");
    ...
    public static final ConfigOption<String> SINK_TYPE =
            key("sink.type")
                    .noDefaultValue()
                    .withDescription(
                            "The type of sink, currently only 'clickhouse' and 'iceberg' are supported");

    // ------------------------------------------------------------------------
    //  Operator parallelism configs
    // ------------------------------------------------------------------------
    public static final ConfigOption<Integer> SOURCE_PARALLELISM =
            key("source.parallelism").defaultValue(1);
    ...
    public static final ConfigOption<Integer> SINK_PARALLELISM =
            key("sink.parallelism").defaultValue(1);

    public static final ConfigOption<Integer> COMMITTER_PARALLELISM =
            key("committer.parallelism").defaultValue(1);
    ...
    public static final ConfigOption<Integer> SINK_HIVE_TEXT_BUFFER_SIZE =
            key("sink.hive.text.buffer.size")
                    .defaultValue(262144)
                    .withDescription(
                            "Buffer size of Hive/THive sink text format (with compression or not), "
    ...
    public static final ConfigOption<Integer> MIN_PAUSE_BETWEEN_CHECKPOINTS_MS =
            key("min.pause.between.checkpoints.ms").defaultValue(500);
    ...
    public static final ConfigOption<Integer> METRICS_AGGREGATOR_WINDOW_SIZE =
            key("metrics.aggregator.window.size").defaultValue(5).withDescription("minutes");

    public static final ConfigOption<String> METRICS_AUDIT_PROXY_HOSTS =
            key("metrics.audit.proxy.hosts")
                    .noDefaultValue()
                    .withDescription(
                            "Audit proxy host address for reporting audit metrics. "
    ...
    public static final ConfigOption<String> DATAFLOW_INFO_FILE =
            key("dataflow.info.file")
                    .noDefaultValue()
                    .withDescription(
                            "The file which contains dataflow info for a single tenant job");
    ...
    public static final ConfigOption<Boolean> LIGHTWEIGHT =
            key("lightweight").defaultValue(false).withDescription("Whether to lightweight or not");
