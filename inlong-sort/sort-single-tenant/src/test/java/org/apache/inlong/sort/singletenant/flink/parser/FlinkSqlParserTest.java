...
public class FlinkSqlParserTest extends AbstractTestBase {
    ...
    public void test() {
        ...
        tableEnv.executeSql("CREATE TABLE pulsar_table (\n"
                + "  `id` STRING,\n"
                + "  `name` STRING\n"
                + ") WITH (\n"
                + "  'connector' = 'pulsar-inlong',\n"
                + "  'generic' = 'true',\n"
                + "  'topic' = 'persistent://public/b_test_thexia/test_stream',\n"
                + "  'value.format' = 'inlong',\n"
                + "  'value.inlong.inner.format' = 'csv',\n"
                + "  'inlong.csv.field-delimiter' = '|',\n"
                + "  'service-url' = 'pulsar://localhost:6650',\n"
                + "  'admin-url' = 'http://localhost:8080',\n"
                + "  'scan.startup.mode' = 'earliest' \n"
                + ")");
        tableEnv.executeSql("select * from pulsar_table ").print();
    }
    ...
    public void testKafka() {
        ...
        tableEnv.executeSql("CREATE TABLE kafka_table (\n"
                ...
                + " 'format' = 'inlong',\n"
                + " 'inlong.inner.format' = 'csv',\n"
                + " 'inlong.csv.ignore-parse-errors' = 'true'\n"
                + ")");
