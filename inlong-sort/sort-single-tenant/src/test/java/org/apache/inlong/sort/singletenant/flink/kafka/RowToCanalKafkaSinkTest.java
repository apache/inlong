...
public class RowToCanalKafkaSinkTest extends KafkaSinkTestBaseForRowData {
    ...
    protected void verifyData(ConsumerRecords<String, Bytes> records) throws IOException {
        ...
        records.forEach(record -> actualResult.add(new String(record.value().get())));
        actualResult.sort(String::compareTo);

        List<String> expectedResult = new ArrayList<>();
        expectedResult.add("{\"data\":[{\"f1\":\"Bob\",\"f2\":80}],\"type\":\"DELETE\"}");
        expectedResult.add("{\"data\":[{\"f1\":\"Tom\",\"f2\":70}],\"type\":\"INSERT\"}");
        expectedResult.add("{\"data\":[{\"f1\":\"Lisa\",\"f2\":90}],\"type\":\"DELETE\"}");
        expectedResult.add("{\"data\":[{\"f1\":\"Anna\",\"f2\":100}],\"type\":\"INSERT\"}");
