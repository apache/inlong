...
//		SerializationSchema<RowData> serializationSchema = getSerializationSchema(
//				context,
//				dataType,
//				schemaMappingMode,
//				physicalSchema
//		);
		return new RedisDynamicTableSink(encodingFormat,physicalSchema,properties
				);
	}

	private SerializationSchema<RowData> getSerializationSchema(
			Context context,
		...
		DataType physicalDataType = physicalSchema.toPhysicalRowDataType();
		int[] projection = {};
		DataType physicalFormatDataType = DataTypeUtils.projectRow(physicalDataType, projection);

		return encodingFormat.createRuntimeEncoder(context, physicalFormatDataType);
	}
	...
	private static Configuration getConfiguration(
		...
		Map<String, String> properties =
				descriptorProperties.getPropertiesWithPrefix(
						RedisValidator.CONNECTOR_PROPERTIES);
	...
	private void validateDataTypeAndSchemaMappingMode(
		...
		switch (dataType) {
			...
			case PLAIN:
				...
				break;
			case BITMAP:
				org.apache.flink.shaded.guava18.com.google.common.base.Preconditions.checkState(mappingMode == STATIC_KV_PAIR,
