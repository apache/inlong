...
/** Elasticsearch 5 specific configuration. */
@Internal
final class Elasticsearch5Configuration extends ElasticsearchConfiguration {
    Elasticsearch5Configuration(ReadableConfig config, ClassLoader classLoader) {
        super(config, classLoader);
    }

    public List<InetSocketAddress>  getHosts() {
        return config.get(HOSTS_OPTION).stream()
                .map(Elasticsearch5Configuration::validateAndParseHostsString)
     ...
     * </pre>
     */
    private static InetSocketAddress  validateAndParseHostsString(String host) {
        try {
