...
public abstract class ElasticsearchSinkTestBase<C extends AutoCloseable, A>
    ...
    private void runElasticSearchSinkTest(
        ...
        source.addSink(
                createElasticsearchSinkForEmbeddedNode(
                        1, getClusterName(), functionFactory.apply(index), null));
    ...
    public void runNullAddressesTest() {
        try {
            createElasticsearchSink(
                    1, getClusterName(), null, SourceSinkDataTestKit.getJsonSinkFunction("test"), null);
     ...
     * ElasticsearchSinkFunction, String)} because the Elasticsearch Java API to do so is incompatible
