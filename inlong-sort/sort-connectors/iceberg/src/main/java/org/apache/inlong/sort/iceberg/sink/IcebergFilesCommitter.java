...
class IcebergFilesCommitter extends AbstractStreamOperator<Void>
    ...
    // increasing, so we
    // could correctly commit all the data files whose checkpoint id is greater than the max
    // committed one to iceberg
    // table, for avoiding committing the same data files twice. This id will be attached to
    // iceberg's meta when
    ...
    // data files
    // <2, <file3>>. Snapshot for checkpoint#1 interrupted because of network/disk failure etc,
    // while we don't expect
    // any data loss in iceberg table. So we keep the finished files <1, <file0, file1>> in memory
    // and retry to commit
    ...
    // snapshot created by
    // the same flink job; another case is restoring from snapshot created by another different job.
    // For the second
    // case, we need to maintain the old flink job's id in flink state backend to find the
    // max-committed-checkpoint-id
    ...
    public void initializeState(StateInitializationContext context) throws Exception {
        ...
        if (context.isRestored()) {
            ...
            // the new flink job even
            // if it's restored from a snapshot created by another different flink job, so it's safe
            // to assign the max
            ...
            if (!uncommittedDataFiles.isEmpty()) {
                ...
                commitUpToCheckpoint(
                        uncommittedDataFiles, restoredFlinkJobId, maxUncommittedCheckpointId);
            }
        }
    }
            ...
            try {
                ...
                String details =
                        MoreObjects.toStringHelper(this)
                                ...
                                .toString();
                LOG.warn(
                        "The iceberg transaction has been committed, but we failed to clean the temporary flink "
                                + "manifests: {}",
                        details,
                        e);
            }
    ...
    private void replacePartitions(
        ...
        Preconditions.checkState(
                deleteFilesNum == 0, "Cannot overwrite partitions with delete files.");
    ...
    private void commitDeltaTxn(
        ...
        if (deleteFilesNum == 0) {
            ...
            for (Map.Entry<Long, WriteResult> e : pendingResults.entrySet()) {
                ...
                // txn1 and txn2, the equality-delete files of txn2 are required to be applied to
                // data files from txn1.
                ...
                // deletes. Equality
                // deletes are applied to data in all previous sequence numbers, so retries may push
                // deletes further in
                ...
                // There is no way for
                // data files added along with the delete files to be concurrently removed, so there
                // is no need to
