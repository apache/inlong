...
public class RecordWithSchema {
    ...
    public RecordWithSchema refreshRowData(BiFunction<JsonNode, Schema, List<RowData>> rowDataExtractor) {
        // Solve the problem of type error during downstream parsing. Here, rowData is
        // set to be compatible with rowType
        data = rowDataExtractor.apply(originalData, schema);
        return this;
    }

    // todo: here RecordWithSchema is deserialized from network, it's `Schema` is
    // the new object, the `Type` is the
    // same.However `Type` do not implement equals method, so some method will
    // return unexpected result when
    // compare this schema with Table#schema loaded from catalog, For example,
    // Schema#sameSchema will return false
    // even thought schema is the same, can't get the comparators of Type even
    // thought type is the same.
