...
public interface TableChange {
    ...
    static List<TableChange> diffSchema(Schema oldSchema, Schema newSchema) {
        ...
        if (oi != oldFields.size()) {
            throw new IllegalArgumentException(String.format("Unspported schema change.\n"
    ...
    static void applySchemaChanges(UpdateSchema pendingUpdate, List<TableChange> tableChanges) {
        for (TableChange change : tableChanges) {
            if (change instanceof TableChange.AddColumn) {
                TableChange.apply(pendingUpdate, (TableChange.AddColumn) change);
            } else {
    ...
    static void apply(UpdateSchema pendingUpdate, TableChange.AddColumn add) {
        Preconditions.checkArgument(add.isNullable(),
                "Incompatible change: cannot add required column: %s", leafName(add.fieldNames));
        pendingUpdate.addColumn(parentName(add.fieldNames()), leafName(add.fieldNames()), add.dataType(), add.comment());
    ...
    static String leafName(String[] fieldNames) {
    ...
    static String peerName(String[] fieldNames, String fieldName) {
    ...
    static String parentName(String[] fieldNames) {
        if (fieldNames.length > 1) {
