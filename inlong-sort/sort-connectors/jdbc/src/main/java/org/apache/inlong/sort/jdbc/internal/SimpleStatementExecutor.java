 ...
 */
class SimpleStatementExecutor<T, V> implements JdbcBatchStatementExecutor<T> {

    private static final Logger LOG = LoggerFactory.getLogger(SimpleStatementExecutor.class);
    ...
    public static <T, V> JdbcBatchStatementExecutor<T> build(
            String sql, JdbcStatementBuilder<V> paramSetter, Function<T, V> valueTransformer) {
        return new SimpleStatementExecutor(sql, paramSetter, valueTransformer);
    }
