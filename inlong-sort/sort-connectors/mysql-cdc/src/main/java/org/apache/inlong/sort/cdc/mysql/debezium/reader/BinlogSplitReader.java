 ...
 * filter overlapping snapshot data that {@link SnapshotSplitReader} read.
 */
public class BinlogSplitReader implements DebeziumReader<SourceRecord, MySqlSplit> {
    ...
    public void submitSplit(MySqlSplit mySqlSplit) {
        ...
        this.capturedTableFilter = statefulTaskContext.getConnectorConfig().getTableFilters().dataCollectionFilter();
        this.queue = statefulTaskContext.getQueue();
        final MySqlOffsetContext mySqlOffsetContext = statefulTaskContext.getOffsetContext();
        this.binlogSplitReadTask = new MySqlBinlogSplitReadTask(
     ...
     *
     * <p>
     * The watermark signal algorithm is the binlog split reader only sends the
     * binlog event that belongs to its finished snapshot splits. For each snapshot
     * split, the binlog event is valid since the offset is after its high
     * watermark.
     * </p>
     *
     * <pre>
    ...
    private boolean shouldEmit(SourceRecord sourceRecord) {
        if (isDataChangeRecord(sourceRecord)) {
            ...
            if (finishedSplitsInfo.containsKey(tableId)) {
                RowType splitKeyType = ChunkUtils.getSplitType(
    ...
    private boolean hasEnterPureBinlogPhase(TableId tableId, BinlogOffset position) {
        ...
        // TODO: there is still very little chance that we can't capture new added
        // table.
        // That the tables dynamically added after discovering captured tables in
        // enumerator
        // and before the lowest binlog offset of all table splits. This interval should
        // be
    ...
    private void configureFilter() {
        List<FinishedSnapshotSplitInfo> finishedSplitInfos = currentBinlogSplit.getFinishedSnapshotSplitInfos();
