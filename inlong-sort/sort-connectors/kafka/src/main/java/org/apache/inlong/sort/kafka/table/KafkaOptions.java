...
public class KafkaOptions {
                    ...
                    .withDescription(
                            String.format(
                                    "Defines a strategy how to deal with key columns in the data type of "
                                            + "the value format. By default, '%s' physical columns of the table schema "
                            ...
                            Description.builder()
                                    ...
                                    .text(
                                            "When constructing the data type of the key format, the prefix "
                                                    + "will be removed and the non-prefixed names"
                                                    + " will be used within the key format.")
                                    .linebreak()
                                    .text(
                                            String.format(
    ...
    public static final ConfigOption<Duration> SCAN_TOPIC_PARTITION_DISCOVERY =
            ConfigOptions.key("scan.topic-partition-discovery.interval")
                    ...
                    .noDefaultValue()
                    .withDescription(
                            "Optional interval for consumer to discover dynamically"
                                    + " created Kafka partitions periodically.");
                            ...
                            Description.builder()
                                    .text(
                                            "The max size of buffered records before flushing. "
                                                    ...
                                                    + "This can help to reduce data shuffling and avoid possible"
                                                    + " tombstone messages to the Kafka topic.")
                                    .linebreak()
    ...
    public static final ConfigOption<Duration> SINK_BUFFER_FLUSH_INTERVAL =
            ConfigOptions.key("sink.buffer-flush.interval")
                    ...
                    .withDescription(
                            Description.builder()
                                    .text(
                                            "The flush interval millis. Over this time, asynchronous threads will "
                                                    + "flush data. When the sink receives many updates on the same "
                                                    + "key, the buffer will retain the last record of the same key.")
                                    .linebreak()
                                    .text("Can be set to '0' to disable it.")
                                    .linebreak()
                                    .text(
                ...
                .ifPresent(
                        partitioner -> {
                            if (partitioner.equals(SINK_PARTITIONER_VALUE_ROUND_ROBIN)
                                ...
                                throw new ValidationException(
                                        "Currently 'round-robin' partitioner only works when option "
                                ...
                                throw new ValidationException(
                                        String.format(
                                                "Unsupported value '%s' for '%s'. Supported values are "
                                                        + "['at-least-once', 'exactly-once', 'none'].",
