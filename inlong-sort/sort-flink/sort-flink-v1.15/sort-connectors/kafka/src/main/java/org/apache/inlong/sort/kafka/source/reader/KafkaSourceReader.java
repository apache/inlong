...
public class KafkaSourceReader<T>
    ...
    public List<KafkaPartitionSplit> snapshotState(long checkpointId) {
        if (metricSchema instanceof DynamicKafkaDeserializationSchema) {
            ((DynamicKafkaDeserializationSchema) metricSchema).updateCurrentCheckpointId(checkpointId);
        }
    ...
    private void flushAudit(long checkpointId) {
        if (metricSchema instanceof DynamicKafkaDeserializationSchema) {
            DynamicKafkaDeserializationSchema schema = (DynamicKafkaDeserializationSchema) metricSchema;
            schema.flushAudit();
