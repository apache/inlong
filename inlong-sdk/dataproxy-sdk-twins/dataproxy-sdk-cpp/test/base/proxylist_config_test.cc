...
TEST(clusterProxylist, readCachelist) //.bus_list.ini文件中缓存有bid_test1和bid_test2
{
    ...
    g_clusters->addBuslist("b_teg_test_robbinli_dr");
    g_clusters->addBuslist("bid_test1");
    g_clusters->addBuslist("bid_test2");
...
TEST(clusterProxylist, basetest)
{
    ...
    string meta_info;
    EXPECT_EQ(Utils::readFile("buslist_old.json", meta_info), true);
    // LOG_INFO("%s", meta_info.c_str());

    EXPECT_EQ(g_cluster->parseAndGet("test_bid", meta_info, cluster), 0);
    ...
    EXPECT_EQ(Utils::readFile("buslist_new.json", info), true);
    EXPECT_EQ(g_cluster->parseAndGet("test_bid", info, cluster2), 0);
...
void addExistBidFunc(GlobalCluster *gcluster)
{
    ...
    gcluster->addBuslist("b_teg_test_robbinli_dr");
}
