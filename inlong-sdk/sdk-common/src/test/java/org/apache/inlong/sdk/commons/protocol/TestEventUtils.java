...
public class TestEventUtils {
        ...
        try {
            ...
            MessagePack packObj =
                    EventUtils.encodeSdkEvents(
                            INLONG_GROUP_ID, INLONG_STREAM_ID, compressedType, eventList);
    ...
    public void testDecodeSdkPack() {
        try {
            ...
            MessagePack packObject =
                    MessagePack.parseFrom(new ByteArrayInputStream(packBytes, 0, packBytes.length));
    ...
    public void testDecodeCacheMessageBody() {
        try {
            ...
            INLONG_COMPRESSED_TYPE compressedType = INLONG_COMPRESSED_TYPE.INLONG_SNAPPY;
            byte[] bodyBytes = EventUtils.encodeCacheMessageBody(compressedType, eventList);
            List<SortEvent> sortEventList =
                    EventUtils.decodeCacheMessageBody(
                            INLONG_GROUP_ID, INLONG_STREAM_ID, compressedType, bodyBytes);
