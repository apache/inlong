...
public class TestMapEntriesFunction extends AbstractFunctionStringTestBase {
    ...
    public void testMapEntriesFunction() throws Exception {
        ...
        transformSql = "select map_entries(map(string1,numeric1,string2)) from source";
        ...
        transformSql = "select map_entries(map(string1,numeric1,string2,string3)) from source";
        config = new TransformConfig(transformSql);
        processor = TransformProcessor
                .create(config, SourceDecoderFactory.createCsvDecoder(csvSource),
                        SinkEncoderFactory.createKvEncoder(kvSink));
        ...
        transformSql = "select map_entries(map(numeric1,numeric2,string1,string2)) from source";
        config = new TransformConfig(transformSql);
        processor = TransformProcessor
                .create(config, SourceDecoderFactory.createCsvDecoder(csvSource),
                        SinkEncoderFactory.createKvEncoder(kvSink));
        ...
        transformSql =
                "select map_entries(map(numeric1,numeric2,map(string1,string2),map(string3,numeric3))) from source";
        config = new TransformConfig(transformSql);
        processor = TransformProcessor
                .create(config, SourceDecoderFactory.createCsvDecoder(csvSource),
                        SinkEncoderFactory.createKvEncoder(kvSink));
