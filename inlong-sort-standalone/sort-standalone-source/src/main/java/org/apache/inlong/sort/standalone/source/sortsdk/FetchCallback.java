 ...
 *
 * <p>The ACKer will be used to <b>ACK</b> upstream after that the downstream <b>ACKed</b>
 * sort-standalone. This process seems like <b>transaction</b> of the whole sort-standalone, and
 * which ensure <b>At Least One</b> semantics.
 */
public class FetchCallback implements ReadCallback {
    ...
    // Temporary usage for ACK. The {@link SortClient} and Callback should not circular reference
    // each other.
     ...
     *
     * <p>The construction of FetchCallback should be initiated by {@link FetchCallback.Factory}.
     *
     ...
     *
     * <p>In order to ACK the fetched msg, {@link FetchCallback} has to hold the {@link SortClient}
     * which results in <b>Cycle Reference</b>. The {@link SortClient} should deliver one object to
     * ACK after invoke the callback method {@link ReadCallback#onFinished(MessageRecord)}.
     *
    ...
    public void onFinished(final MessageRecord messageRecord) {
        try {
            ...
            CacheMessageRecord cacheRecord =
                    new CacheMessageRecord(
                            messageRecord, client, CommonPropertiesHolder.getAckPolicy());
            for (InLongMessage inLongMessage : messageRecord.getMsgs()) {
                ...
                context.reportToMetric(
                        profileEvent, sortTaskName, "-", SortSdkSourceContext.FetchResult.SUCCESS);
            }
        } catch (NullPointerException npe) {
            LOG.error("Got a null pointer exception for sort task " + sortTaskName, npe);
            context.reportToMetric(
                    null, sortTaskName, "-", SortSdkSourceContext.FetchResult.FAILURE);
        } catch (Exception e) {
    ...
    public static class Factory {
         ...
         *
         * <p>Validate sortId and channelProcessor before the construction of FetchCallback.
         *
        ...
        public static FetchCallback create(
                ...
                @NotNull(message = "channelProcessor should not be null.")
                        final ChannelProcessor channelProcessor,
                @NotNull(message = "context should not be null")
                        final SortSdkSourceContext context) {
