    ...
    private boolean initEsclient() {
        try {
            if (esClient == null || !esClient.ping(RequestOptions.DEFAULT)) {
                ...
                LOG.info(
                        "initEsclient:url:{},user:{},password:{}",
                ...
                builder.setHttpClientConfigCallback(
                        (httpAsyncClientBuilder) -> {
                            ...
                            RequestConfig requestConfig =
                                    RequestConfig.custom().setConnectTimeout(120 * 1000).build();
        ...
        try {
            if (bulkProcessor == null) {
                ...
                BulkProcessor bulkProcessor =
                        BulkProcessor.builder(consumer, esListener)
                                .setBulkActions(context.getBulkAction())
                                .setBulkSize(
                                        new ByteSizeValue(context.getBulkSizeMb(), ByteSizeUnit.MB))
                                .setFlushInterval(
                                        TimeValue.timeValueSeconds(context.getFlushInterval()))
