    ...
    public void run() {
        ...
        try (Connection conn = context.getHiveConnection()) {
            ...
            for (Entry<String, HdfsIdConfig> entry : idConfigMap.entrySet()) {
                LOG.info(
                        "start to PartitionLeaderElectionRunnable check id token:{}",
                        entry.getKey());
                if (hasToken(entry.getValue())) {
                    ...
                    long beginScanTime =
                            currentTime
                                    - 2
                                            * idConfig.getMaxPartitionOpenDelayHour()
                                            * HdfsIdConfig.HOUR_MS;
                    long endScanTime =
                            currentTime
                                    - context.getMaxFileOpenDelayMinute()
                                            * HiveSinkContext.MINUTE_MS;
                    long forceCloseTime =
                            currentTime
                                    - idConfig.getMaxPartitionOpenDelayHour()
                                            * HdfsIdConfig.HOUR_MS;
                    LOG.info(
                            "start to PartitionLeaderElectionRunnable scan:beginScanTime:{},"
                                    + "endScanTime:{},getPartitionIntervalMs:{}",
                            beginScanTime,
                            endScanTime,
                            idConfig.getPartitionIntervalMs());
                    for (long pt = beginScanTime;
                            pt < endScanTime;
                            pt += idConfig.getPartitionIntervalMs()) {
                        ...
                        PartitionCreateRunnable createTask =
                                this.partitionCreateMap.get(uidPartitionKey);
                        LOG.info(
                                "start to PartitionLeaderElectionRunnable createTask:{},isForce:{}",
                                uidPartitionKey,
                                isForce);
                        if (createTask != null) {
                        ...
                        createTask =
                                new PartitionCreateRunnable(
                                        context, idConfig, strPartitionValue, pt, isForce);
    ...
    private boolean hasToken(HdfsIdConfig idConfig) {
        ...
        try {
            ...
            if (!fs.exists(tokenPath)) {
                ...
                fsdos.close();
                LOG.info(
                        "node:{} get id token:inlongGroupId:{},inlongStreamId:{} because token file is not existed.",
                        containerName,
                ...
                if (currentTime - modifiedTime < tokenOvertime
                    ...
                    LOG.info(
                            "node:{},leader:{},containerNameLength:{},leaderNameLength:{},"
                                    + "leaderBytesLength:{}",
                            containerName,
                            leaderName,
                    ...
                    if (leaderName.equals(containerName)) {
                        ...
                        // "leader rewrite file.";
                        LOG.info(
                                "node:{} get id token:inlongGroupId:{},inlongStreamId:{} "
                        ...
                        LOG.info(
                                "node:{},leader:{},inlongGroupId:{},inlongStreamId:{} "
                    ...
                    LOG.info(
                            "node:{} get id token:inlongGroupId:{},inlongStreamId:{} because leader is overtime:"
                                    + "currentTime:{},accessTime:{},modifiedTime:{},tokenOvertime:{}.",
                            containerName,
                            idConfig.getInlongGroupId(),
                            idConfig.getInlongStreamId(),
            ...
            LOG.error(
                    "node:{} is fail to get id token:inlongGroupId:{},inlongStreamId:{} because of error:{}",
                    containerName,
                    idConfig.getInlongGroupId(),
                    idConfig.getInlongStreamId(),
                    e);
