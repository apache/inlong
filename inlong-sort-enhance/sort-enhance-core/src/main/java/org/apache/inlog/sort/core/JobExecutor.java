...
package org.apache.inlog.sort.core;
...
import org.apache.inlog.sort.core.util.JobStatementSplitter;

import java.util.List;

public class JobExecutor {

  public static void runJob(JobInfo jobInfo) throws Exception {

    StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
    EnvironmentSettings fsSettings = EnvironmentSettings.newInstance().inStreamingMode().build();
    StreamTableEnvironment tEnv = StreamTableEnvironment.create(env, fsSettings);

    StatementSet statementSet = handleStatements(jobInfo.getWorkFlowContent(), tEnv);
  ...
  private static StatementSet handleStatements(String workFlowContent, StreamTableEnvironment tEnv) {
    ...
    List<String> stmts = JobStatementSplitter.normalizeStatements(workFlowContent);
