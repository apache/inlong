...
public class TestConfiguration {
    ...
    public void testString() throws Exception {
        assertEquals("tdm", agentConf.get("agent.conf.resource", "none"));
    ...
    public void testJobConf() throws Exception {
        ...
        Assert.assertEquals("1", jobConf.get(JobConstants.JOB_ID));
        Assert.assertEquals("test", jobConf.get(JobConstants.JOB_NAME));

        Assert.assertTrue(jobJsonConf.allRequiredKeyExist());
        Assert.assertEquals("1", jobJsonConf.get(JobConstants.JOB_ID));
        Assert.assertEquals("test", jobJsonConf.get(JobConstants.JOB_NAME));
    }
