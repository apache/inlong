...
public class PostgreSQLJdbcUtils {
    ...
    public static Connection getConnection(String url, String user, String password)
        ...
        if (StringUtils.isBlank(url) || !url.startsWith(POSTGRES_JDBC_PREFIX)) {
            throw new Exception(
                    "PostgreSQL server URL was invalid, it should start with jdbc:postgresql");
        }
        Connection conn;
        try {
    ...
    public static boolean checkColumnExist(
        ...
        final String checkColumnSql =
                PostgreSQLSqlBuilder.getCheckColumn(schemaName, tableName, column);
        try (Statement statement = conn.createStatement();
                ResultSet resultSet = statement.executeQuery(checkColumnSql)) {
            if (Objects.nonNull(resultSet) && resultSet.next()) {
                int count = resultSet.getInt(1);
    ...
    public static void createTable(final Connection conn, final PostgreSQLTableInfo tableInfo)
        ...
        if (checkTablesExist(conn, tableInfo.getSchemaName(), tableInfo.getTableName())) {
            ...
            final List<String> createTableSqls =
                    PostgreSQLSqlBuilder.buildCreateTableSql(tableInfo);
     ...
     */
    public static List<PostgreSQLColumnInfo> getColumns(
            final Connection conn, final String schemaName, final String tableName)
            throws Exception {
    ...
    public static void addColumns(
            ...
            final String schemaName,
            final String tableName,
            final List<PostgreSQLColumnInfo> columns)
            throws Exception {
        ...
        final List<String> addColumnSql =
                PostgreSQLSqlBuilder.buildAddColumnsSql(schemaName, tableName, columnInfos);
