...
public class PulsarOperator {
    ...
    @Autowired private ConversionHandle conversionHandle;
    ...
    public void createNamespace(
            PulsarAdmin pulsarAdmin, InlongPulsarInfo pulsarInfo, String tenant, String namespace)
            throws PulsarAdminException {
        Preconditions.checkNotNull(tenant, "pulsar tenant cannot be empty during create namespace");
        Preconditions.checkNotNull(
                namespace, "pulsar namespace cannot be empty during create namespace");
        ...
        try {
            ...
            if (ttl > 0) {
                namespaces.setNamespaceMessageTTL(
                        namespaceName,
                        conversionHandle.handleConversion(
                                ttl, pulsarInfo.getTtlUnit().toLowerCase() + "_seconds"));
            }
            ...
            PersistencePolicies persistencePolicies =
                    new PersistencePolicies(
                            pulsarInfo.getEnsemble(),
                            pulsarInfo.getWriteQuorum(),
                            pulsarInfo.getAckQuorum(),
                            0);
    ...
    /** Create Pulsar topic */
    public void createTopic(PulsarAdmin pulsarAdmin, PulsarTopicBean topicBean)
            throws PulsarAdminException {
        ...
        if (topicIsExists(
            ...
            LOGGER.warn(
                    "pulsar topic={} already exists in {}",
                    topicFullName,
        ...
        try {
            if (InlongConstants.PULSAR_QUEUE_TYPE_SERIAL.equals(topicBean.getQueueModule())) {
                ...
                Map<String, String> res =
                        pulsarAdmin.lookups().lookupPartitionedTopic(topicFullName);
                ...
                if (res.keySet().size() != numPartitions) {
                    ...
                    for (int i = 0;
                            (i < RETRY_TIMES && res.keySet().size() != numPartitions);
                            i++) {
    ...
    public void forceDeleteTopic(PulsarAdmin pulsarAdmin, PulsarTopicBean topicBean)
            throws PulsarAdminException {
        Preconditions.checkNotNull(topicBean, "pulsar topic info cannot be empty");
    ...
    public void createSubscription(
            PulsarAdmin pulsarAdmin, PulsarTopicBean topicBean, String subscription)
            throws PulsarAdminException {
        Preconditions.checkNotNull(
                topicBean, "can not find tenant information to create subscription");
        Preconditions.checkNotNull(
                subscription, "subscription cannot be empty during creating subscription");

        String topicName =
                topicBean.getTenant()
                        + "/"
                        + topicBean.getNamespace()
                        + "/"
                        + topicBean.getTopicName();
        LOGGER.info("begin to create pulsar subscription={} for topic={}", subscription, topicName);
        try {
            boolean isExists =
                    this.subscriptionIsExists(
                            pulsarAdmin,
                            topicName,
                            subscription,
                            InlongConstants.PULSAR_QUEUE_TYPE_PARALLEL.equals(
                                    topicBean.getQueueModule()));
            if (isExists) {
    ...
    public void createSubscriptions(
        ...
        LOGGER.info(
                "success to create subscription={} for multiple topics={}",
                subscription,
                topicList);
    }
        ...
        try {
            ...
            try {
                while (!topicExists && ++count <= RETRY_TIMES) {
                    LOGGER.info(
                            "check whether the pulsar topic={} exists error, try count={}",
                            topic,
                            count);
                ...
                LOGGER.error(
                        "after retry, check if the pulsar topic={} exists still error", topic, pe);
            }
        }
        return topicExists;
    }

    private boolean subscriptionIsExists(
            PulsarAdmin pulsarAdmin, String topic, String subscription, boolean isPartitioned) {
        int count = 0;
        while (++count <= RETRY_TIMES) {
            try {
                LOGGER.info(
                        "check whether the subscription exists for topic={}, try count={}",
                        topic,
                        count);
                Thread.sleep(DELAY_SECONDS);
                ...
                LOGGER.error(
                        "check if the subscription exists for topic={} error, continue retry",
                        topic,
                        e);
                if (count == RETRY_TIMES) {
                    LOGGER.error(
                            "after {} times retry, still check subscription exception for topic {}",
                            count,
                            topic);
