...
public class TubeMQOperator {
    ...
    public void createTopic(
            @Nonnull TubeClusterInfo tubeCluster, String topicName, String operator) {
        ...
        if (this.isTopicExist(masterUrl, topicName)) {
            LOGGER.warn(
                    "tubemq topic {} already exists in {}, skip to create", topicName, masterUrl);
            return;
        }
    ...
    public void createConsumerGroup(
            TubeClusterInfo tubeCluster, String topic, String consumerGroup, String operator) {
        String masterUrl = tubeCluster.getMasterWebUrl();
        LOGGER.info(
                "begin to create consumer group {} for topic {} in master {}",
                consumerGroup,
                topic,
                masterUrl);
        if (StringUtils.isEmpty(masterUrl)
            ...
            throw new BusinessException(
                    "tubemq master url, consumer group, or tubemq topic cannot be null");
        }

        if (!this.isTopicExist(masterUrl, topic)) {
            LOGGER.warn(
                    "cannot create tubemq consumer group {}, as the topic {} not exists in master {}",
                    consumerGroup,
                    topic,
                    masterUrl);
            return;
        ...
        if (this.isConsumerGroupExist(masterUrl, topic, consumerGroup)) {
            LOGGER.warn(
                    "tubemq consumer group {} already exists for topic {} in master {}, skip to create",
                    consumerGroup,
                    topic,
                    masterUrl);
            return;
        ...
        this.createConsumerGroupOpt(
                masterUrl, topic, consumerGroup, tubeCluster.getToken(), operator);
        LOGGER.info(
                "success to create tubemq consumer group {} for topic {} in {}",
                consumerGroup,
                topic,
                masterUrl);
    }
    ...
    public boolean isConsumerGroupExist(String masterUrl, String topicName, String consumerGroup) {
        LOGGER.info(
                "begin to check if the consumer group {} exists on topic {}",
                consumerGroup,
                topicName);
        String url =
                masterUrl
                        ...
                        + consumerGroup;
        try {
            ConsumerGroupResponse response =
                    HttpUtils.request(
                            restTemplate,
                            url,
                            HttpMethod.GET,
            ...
            if (CollectionUtils.isEmpty(response.getData())) {
                LOGGER.warn(
                        "tubemq consumer group {} not exists for topic {} in {}",
                        consumerGroup,
                        topicName,
                        url);
                return false;
            ...
            LOGGER.info(
                    "tubemq consumer group {} exists for topic {} in {}",
                    consumerGroup,
                    topicName,
                    url);
            return true;
            ...
            String msg =
                    String.format(
                            "failed to check if the consumer group %s for topic %s exist in ",
    ...
    private TubeBrokerInfo getBrokerInfo(String masterUrl) {
        ...
        try {
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(
                        "success to query tubemq broker from {}, result {}",
                        url,
                        brokerInfo.getData());
            }
            return brokerInfo;
            ...
            throw new BusinessException(
                    String.format(msg, masterUrl) + ", error: " + e.getMessage());
        }
    }
    ...
    private void createTopicOpt(String masterUrl, String topicName, String token, String operator) {
        LOGGER.info(
                String.format(
                        "begin to create tubemq topic %s in master %s", topicName, masterUrl));
        ...
        if (CollectionUtils.isEmpty(allBrokers)) {
            String msg =
                    String.format(
                            "cannot create topic %s, as not any brokers found in %s",
                            topicName, masterUrl);
            LOGGER.error(msg);
        ...
        String url =
                masterUrl
                        ...
                        + topicName
                        + BROKER_ID
                        + StringUtils.join(allBrokers, ",")
                        + CREATE_USER
                        + operator
                        + CONF_MOD_AUTH_TOKEN
                        + token;
        try {
            TubeHttpResponse response =
                    HttpUtils.request(
                            restTemplate,
                            url,
                            HttpMethod.GET,
            ...
            if (response.getErrCode() != SUCCESS_CODE) {
                String msg =
                        String.format(
                                "failed to create tubemq topic %s, error: %s",
            ...
            String msg =
                    String.format("failed to create tubemq topic %s in %s", topicName, masterUrl);
            LOGGER.error(msg, e);
    ...
    private void createConsumerGroupOpt(
        ...
        LOGGER.info(
                String.format(
                        "begin to create consumer group %s for topic %s in master %s",
        ...
        try {
            ...
            if (response.getErrCode() != SUCCESS_CODE) {
                String msg =
                        String.format(
                                "failed to create tubemq consumer group %s for topic %s, error: %s",
