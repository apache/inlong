...
public class PulsarResourceOperator implements QueueResourceOperator {
    ...
    @Autowired private PulsarOperator pulsarOperator;
    ...
    public void createQueueForGroup(InlongGroupInfo groupInfo, String operator) {
        ...
        PulsarClusterInfo pulsarCluster =
                (PulsarClusterInfo) clusterService.getOne(clusterTag, null, ClusterType.PULSAR);
        try (PulsarAdmin pulsarAdmin = PulsarUtils.getPulsarAdmin(pulsarCluster)) {
            ...
            if (!Objects.equal(GroupStatus.CONFIG_SUCCESSFUL.getCode(), groupInfo.getStatus())) {
                ...
                log.info(
                        "success to create pulsar tenant for groupId={}, tenant={}",
                        groupId,
                        tenant);
                pulsarOperator.createNamespace(pulsarAdmin, pulsarInfo, tenant, namespace);
                log.info(
                        "success to create pulsar namespace for groupId={}, namespace={}",
                        groupId,
                        namespace);
            }
            ...
            if (streamInfoList == null || streamInfoList.isEmpty()) {
                log.warn(
                        "skip to create pulsar topic and subscription as no streams for groupId={}",
                        groupId);
                return;
            }
        ...
        log.info(
                "success to create pulsar resource for groupId={}, cluster={}",
                groupId,
                pulsarCluster);
    }
    ...
    public void deleteQueueForGroup(InlongGroupInfo groupInfo, String operator) {
        ...
        try {
            ...
            for (InlongStreamBriefInfo streamInfo : streamInfoList) {
                this.deletePulsarTopic(
                        groupInfo, (PulsarClusterInfo) clusterInfo, streamInfo.getMqResource());
            }
        } catch (Exception e) {
            log.error("failed to delete pulsar resource for groupId=" + groupId, e);
            throw new WorkflowListenerException(
                    "failed to delete pulsar resource: " + e.getMessage());
        }

        log.info(
                "success to delete pulsar resource for groupId={}, cluster={}",
                groupId,
                clusterInfo);
    }
    ...
    public void createQueueForStream(
        ...
        try {
            ...
            // create pulsar topic
            this.createPulsarTopic(
                    groupInfo, (PulsarClusterInfo) clusterInfo, streamInfo.getMqResource());
        } catch (Exception e) {
            String msg =
                    String.format(
                            "failed to create pulsar topic for groupId=%s, streamId=%s",
                            groupId, streamId);
            log.error(msg, e);
            throw new WorkflowListenerException(msg + ": " + e.getMessage());
        }
        ...
        log.info(
                "success to create pulsar resource for groupId={}, streamId={}", groupId, streamId);
    }
    ...
    public void deleteQueueForStream(
        ...
        try {
            ...
            this.deletePulsarTopic(
                    groupInfo, (PulsarClusterInfo) clusterInfo, streamInfo.getMqResource());
            log.info(
                    "success to delete pulsar topic for groupId={}, streamId={}",
                    groupId,
                    streamId);
        } catch (Exception e) {
            String msg =
                    String.format(
                            "failed to delete pulsar topic for groupId=%s, streamId=%s",
                            groupId, streamId);
            log.error(msg, e);
        ...
        log.info(
                "success to delete pulsar resource for groupId={}, streamId={}", groupId, streamId);
    }

    /** Create Pulsar Topic and Subscription, and save the consumer group info. */
    private void createPulsarTopic(
            InlongGroupInfo groupInfo, PulsarClusterInfo pulsarCluster, String topicName)
            throws Exception {
        try (PulsarAdmin pulsarAdmin = PulsarUtils.getPulsarAdmin(pulsarCluster)) {
            ...
            if (!exist) {
                ...
                throw new WorkflowListenerException(
                        "topic=" + topicFullName + " not exists in " + serviceUrl);
            }

            // subscription naming rules: clusterTag_topicName_consumer_group
            String subscription =
                    groupInfo.getInlongClusterTag() + "_" + topicName + "_consumer_group";
            pulsarOperator.createSubscription(pulsarAdmin, topicBean, subscription);
            String groupId = groupInfo.getInlongGroupId();
            log.info(
                    "success to create pulsar subscription for groupId={}, topic={}, subs={}",
                    groupId,
                    topicName,
                    subscription);
            ...
            log.info(
                    "success to save consume for groupId={}, topic={}, subs={}",
                    groupId,
                    topicName,
                    subscription);
        }
    ...
    private void deletePulsarTopic(
            InlongGroupInfo groupInfo, PulsarClusterInfo pulsarCluster, String topicName)
            throws Exception {
        try (PulsarAdmin pulsarAdmin = PulsarUtils.getPulsarAdmin(pulsarCluster)) {
