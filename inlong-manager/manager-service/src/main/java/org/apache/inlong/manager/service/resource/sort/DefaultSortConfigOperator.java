...
public class DefaultSortConfigOperator implements SortConfigOperator {
    ...
    @Autowired private StreamTransformService transformService;
    ...
    public void buildConfig(
        ...
        if (groupInfo == null || CollectionUtils.isEmpty(streamInfos)) {
            LOGGER.warn(
                    "group info is null or stream infos is empty, no need to build sort config for disable zk");
            return;
        }
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(
                    "success to build sort config, isStream={}, dataflow={}", isStream, dataflow);
        }
    }

    private GroupInfo getLightweightGroupInfo(
                ...
                transformResponses.stream()
                        .collect(
                                Collectors.groupingBy(
                                        TransformResponse::getInlongStreamId,
                                        HashMap::new,
        ...
        for (InlongStreamInfo inlongStream : streamInfoList) {
            ...
            if (CollectionUtils.isNotEmpty(transformResponseList)) {
                transformResponseList.forEach(
                        trans ->
                                parseConstantFieldMap(
                                        trans.getTransformName(), trans.getFieldList(), fieldMap));
            }

            // build a stream info from the nodes and relations
            List<Node> nodes =
                    this.createNodesWithTransform(
                            sourceMap.get(streamId),
     ...
     */
    private GroupInfo getStandardGroupInfo(
            InlongGroupInfo groupInfo, List<InlongStreamInfo> streamInfoList) {
        // get source info
        Map<String, List<StreamSource>> sourceMap =
                sourceService.getSourcesMap(groupInfo, streamInfoList);
    ...
    private List<Node> createNodesWithoutTransform(
    ...
    private List<Node> createNodesWithTransform(
            List<StreamSource> sources,
            List<TransformResponse> transformResponses,
            List<StreamSink> sinks,
            Map<String, StreamField> constantFieldMap) {
     ...
     */
    private void parseConstantFieldMap(
            String nodeId, List<StreamField> fields, Map<String, StreamField> constantFieldMap) {
    ...
    private void addToStreamExt(List<InlongStreamInfo> streamInfos, String value) {
        streamInfos.forEach(
                streamInfo -> {
                    ...
                    streamInfo
                            .getExtList()
                            .removeIf(ext -> extInfo.getKeyName().equals(ext.getKeyName()));
