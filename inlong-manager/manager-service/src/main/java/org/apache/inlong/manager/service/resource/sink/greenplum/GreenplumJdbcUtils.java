...
public class GreenplumJdbcUtils {
    ...
    public static Connection getConnection(
        ...
        if (StringUtils.isBlank(url) || !url.startsWith(GREENPLUM_JDBC_PREFIX)) {
            throw new Exception(
                    "Greenplum server URL was invalid, it should start with jdbc:postgresql");
        }
        ...
        try {
            ...
            final String errorMsg =
                    "get Greenplum connection error, please check postgres jdbc url,"
    ...
    public static void createSchema(
            final Connection conn, final String schemaName, final String userName)
            throws Exception {
    ...
    public static boolean checkColumnExist(
            ...
            throws Exception {
        boolean result = false;
        final String checkColumnSql =
                GreenplumSqlBuilder.getCheckColumn(schemaName, tableName, column);
        try (Statement statement = conn.createStatement();
     ...
     * @throws Exception on check column exist error
     */
    public static boolean checkSchemaExist(final Connection conn, final String schemaName)
            throws Exception {
        boolean result = false;
     ...
     */
    public static List<GreenplumColumnInfo> getColumns(
            final Connection conn, final String schemaName, final String tableName)
            throws Exception {
        ...
        try (Statement stmt = conn.createStatement();
            ...
            while (rs.next()) {
                columnList.add(
                        new GreenplumColumnInfo(rs.getString(1), rs.getString(2), rs.getString(3)));
            }
        }
        return columnList;
    }
    ...
    public static void addColumns(
            ...
            final String schemaName,
            final String tableName,
            final List<GreenplumColumnInfo> columns)
            throws Exception {
        ...
        final List<String> addColumnSql =
                GreenplumSqlBuilder.buildAddColumnsSql(schemaName, tableName, columnInfos);
