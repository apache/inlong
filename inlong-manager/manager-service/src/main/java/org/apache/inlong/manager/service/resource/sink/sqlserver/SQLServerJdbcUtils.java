...
public class SQLServerJdbcUtils {
    ...
    private static final String SQLSERVER_DRIVER_CLASS =
    ...
    public static Connection getConnection(
        ...
        if (StringUtils.isBlank(url) || !url.startsWith(SQLSERVER_JDBC_PREFIX)) {
            throw new Exception(
                    "SQLServer server URL was invalid, it should start with jdbc:sqlserver");
        }
        Connection conn;
        try {
            ...
            final String errorMsg =
                    "get SQLServer connection error, please check SQLServer JDBC url, "
     ...
     */
    public static void createSchema(final Connection conn, final String schemaName)
            throws Exception {
    ...
    public static boolean checkTablesExist(
        ...
        LOG.info(
                "check table exist for schema={} table={}, result={}",
                schemaName,
                tableName,
                result);
        return result;
    ...
    public static boolean checkColumnExist(
        ...
        final String checkTableSql =
                SQLServerSqlBuilder.getCheckColumn(schemaName, tableName, column);
        try (Statement statement = conn.createStatement();
                ResultSet resultSet = statement.executeQuery(checkTableSql)) {
            if (null != resultSet && resultSet.next()) {
                int count = resultSet.getInt(1);
        ...
        LOG.info(
                "check column exist for schema={} table={}, result={} column={}",
                schemaName,
                tableName,
                result,
                column);
     ...
     */
    public static List<SQLServerColumnInfo> getColumns(
            final Connection conn, final String schemaName, final String tableName)
            throws Exception {
    ...
    public static void addColumns(
            ...
            final String schemaName,
            final String tableName,
            List<SQLServerColumnInfo> columns)
            throws Exception {
        ...
        executeSqlBatch(
                conn, SQLServerSqlBuilder.buildAddColumnsSql(schemaName, tableName, columnInfos));
    }
}
