...
public class SuspendSortListener implements SortOperateListener {
    ...
    public boolean accept(WorkflowContext workflowContext) {
        ...
        if (!(processForm instanceof GroupResourceProcessForm)) {
            log.info(
                    "not add suspend group listener, not GroupResourceProcessForm for groupId [{}]",
                    groupId);
            return false;
        }
        ...
        if (groupProcessForm.getGroupOperateType() != GroupOperateType.SUSPEND) {
            log.info(
                    "not add suspend group listener, as the operate was not SUSPEND for groupId [{}]",
                    groupId);
            return false;
        }
    ...
    public ListenerResult listen(WorkflowContext context) throws Exception {
        ...
        if (StringUtils.isEmpty(sortExt)) {
            String message =
                    String.format(
                            "suspend sort failed for groupId [%s], as the sort properties is empty",
                            groupId);
            log.error(message);
            return ListenerResult.fail(message);
        }
