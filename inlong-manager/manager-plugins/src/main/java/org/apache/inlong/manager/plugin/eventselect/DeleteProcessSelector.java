...
public class DeleteProcessSelector implements EventSelector {
    ...
    public boolean accept(WorkflowContext context) {
        ...
        if (!(processForm instanceof GroupResourceProcessForm)) {
            log.info("not add deleteProcess listener a GroupResourceProcessForm for groupId [{}]",
                    groupId);
            return false;
        }
        ...
        if (!flag) {
            log.info("not add deleteProcess listener as the operate  DELETE for groupId [{}]", groupId);
            return false;
        }
