...
public class DeleteSortListener implements SortOperateListener {
    ...
    public boolean accept(WorkflowContext context) {
        ...
        if (!(processForm instanceof GroupResourceProcessForm)) {
            log.info(
                    "not add delete group listener, not GroupResourceProcessForm for groupId [{}]",
                    groupId);
            return false;
        }
        ...
        if (groupProcessForm.getGroupOperateType() != GroupOperateType.DELETE) {
            log.info(
                    "not add delete group listener, as the operate was not DELETE for groupId [{}]",
                    groupId);
            return false;
        }
    ...
    public ListenerResult listen(WorkflowContext context) throws Exception {
        ...
        if (StringUtils.isEmpty(sortExt)) {
            String message =
                    String.format(
                            "delete sort failed for groupId [%s], as the sort properties is empty",
                            groupId);
            log.error(message);
            return ListenerResult.fail(message);
        }
