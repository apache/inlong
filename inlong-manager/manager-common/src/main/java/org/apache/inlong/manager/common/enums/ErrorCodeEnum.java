...
public enum ErrorCodeEnum {
    ...
    GROUP_ID_UPDATE_NOT_ALLOWED(
            1008, "The current inlong group status does not support modifying the group id"),
    GROUP_MIDDLEWARE_UPDATE_NOT_ALLOWED(
            1011, "The current inlong group status does not support modifying the MQ type"),
    GROUP_NAME_UPDATE_NOT_ALLOWED(
            1012, "The current inlong group status does not support modifying the name"),
    GROUP_INFO_INCONSISTENT(
            1013, "The inlong group info is inconsistent, please contact the administrator"),
    GROUP_MODE_UNSUPPORTED(
            1014, "The current inlong group mode only support lightweight, standard"),

    OPT_NOT_ALLOWED_BY_STATUS(
            1021, "InlongGroup status %s was not allowed to add/update/delete related info"),
    ...
    STREAM_DELETE_HAS_SOURCE(
            1209, "The inlong stream contains source info and is not allowed to be deleted"),
    STREAM_DELETE_HAS_SINK(
            1210, "The inlong stream contains data sink info and is not allowed to be deleted"),
    ...
    SOURCE_OPT_NOT_ALLOWED(
            1306, "Current status does not allow add/modification/delete source info"),
    ...
    SINK_DB_NAME_UPDATE_NOT_ALLOWED(
            1408, "Current status does not allow modification the database name"),
    SINK_TB_NAME_UPDATE_NOT_ALLOWED(
            1409, "Current status does not allow modification the table name"),
    ...
    PARTITION_FIELD_NO_SOURCE_FIELD(
            1414, "Sink partition field [%s] must have a related source field name"),
    ...
    PULSAR_DLQ_RLQ_ERROR(
            2606, "Wrong config for the RLQ and DLQ: RLQ was enabled, but the DLQ was disabled"),
