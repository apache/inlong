...
public interface StreamSourceEntityMapper {
    ...
    List<StreamSourceEntity> selectByRelatedId(
            @Param("groupId") String groupId,
            @Param("streamId") String streamId,
            @Param("sourceName") String sourceName);

    /** Query the tasks by the given status list. */
    List<StreamSourceEntity> selectByStatus(
            @Param("statusList") List<Integer> list, @Param("limit") int limit);

    /** Query the tasks by the given status list and type List. */
    List<StreamSourceEntity> selectByStatusAndType(
            @Param("statusList") List<Integer> statusList,
            @Param("sourceTypeList") List<String> sourceTypeList,
            @Param("limit") int limit);

    /** Query the tasks by the given status list and type List. */
    List<StreamSourceEntity> selectByAgentIpOrCluster(
            @Param("statusList") List<Integer> statusList,
            @Param("sourceTypeList") List<String> sourceTypeList,
     ...
     *
     * @apiNote Sources with is_deleted > 0 should also be returned to agents to clear their local
     *     tasks.
     */
    List<StreamSourceEntity> selectByStatusAndIp(
            @Param("statusList") List<Integer> statusList,
            @Param("agentIp") String agentIp,
