...
public class TransformDefinitionTest {
    ...
    public void testParseDeDuplicationDefinition() {
        ...
        StreamField timingField =
                new StreamField(2, FieldType.TIMESTAMP.toString(), "event_time", null, null);
        DeDuplicationDefinition deDuplicationDefinition =
                new DeDuplicationDefinition(
                        streamFields,
                        timingField,
                        100,
                        TimeUnit.MICROSECONDS,
                        DeDuplicationStrategy.RESERVE_FIRST);
        String definitionJson = gson.toJson(deDuplicationDefinition);
        DeDuplicationDefinition parsedDefinition =
                gson.fromJson(definitionJson, DeDuplicationDefinition.class);
        Assertions.assertEquals(
                deDuplicationDefinition.getDupFields().size(),
                parsedDefinition.getDupFields().size());
    }
    ...
    public void testParseFilterDefinition() {
        ...
        FilterDefinition filterDefinition =
                new FilterDefinition(FilterStrategy.RETAIN, filterRules);
        ...
        Assertions.assertEquals(
                filterDefinition.getFilterRules().size(), parsedDefinition.getFilterRules().size());
    }
    ...
    public void testJoinerDefinition() {
        ...
        JoinerDefinition joinerDefinition =
                new JoinerDefinition(
                        leftNode, rightNode, streamFields, streamFields, JoinMode.INNER_JOIN);
        ...
        Assertions.assertEquals(
                joinerDefinition.getLeftJoinFields().size(),
                parsedDefinition.getLeftJoinFields().size());
        Assertions.assertEquals(
                joinerDefinition.getRightJoinFields().size(),
    ...
    public void testEncryptDefinition() {
        ...
        Assertions.assertEquals(
                encryptDefinition.getEncryptRules().size(),
                parsedDefinition.getEncryptRules().size());
    }
    ...
    private List<EncryptRule> createEncryptRule() {
        ...
        encryptRules.add(
                new EncryptRule(
                        new StreamField(1, FieldType.INT.toString(), "age", null, null), "2", "1"));
        return encryptRules;
    }
}
