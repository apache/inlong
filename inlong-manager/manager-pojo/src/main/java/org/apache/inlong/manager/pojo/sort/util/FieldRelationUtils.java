...
public class FieldRelationUtils {
    ...
    public static List<FieldRelation> createFieldRelations(
        ...
        switch (transformType) {
            case SPLITTER:
                ...
                return createSplitterFieldRelations(
                        fieldList, transformName, splitterDefinition, preNodes, constantFieldMap);
            case STRING_REPLACER:
                StringReplacerDefinition replacerDefinition =
                        (StringReplacerDefinition) transformDefinition;
                return createReplacerFieldRelations(
                        fieldList, transformName, replacerDefinition, preNodes, constantFieldMap);
            case ENCRYPT:
                EncryptDefinition encryptDefinition = (EncryptDefinition) transformDefinition;
                return createEncryptFieldRelations(
                        fieldList, transformName, encryptDefinition, preNodes, constantFieldMap);
            case DE_DUPLICATION:
            case FILTER:
    ...
    /** Create relation of fields. */
    private static List<FieldRelation> createFieldRelations(
            List<StreamField> fieldList, Map<String, StreamField> constantFieldMap) {
                ...
                .map(
                        fieldInfo -> {
                            ...
                            FieldInfo outputField =
                                    new FieldInfo(fieldInfo.getName(), fieldInfo.getFormatInfo());
    ...
    private static List<FieldRelation> createJoinerFieldRelations(
            List<StreamField> fieldList, Map<String, StreamField> constantFieldMap) {
        return fieldList.stream()
                .map(
                        streamField -> {
                            ...
                            String fieldKey =
                                    String.format(
                                            "%s-%s",
                                            streamField.getOriginNodeName(),
                                            streamField.getOriginFieldName());
                            StreamField constantField = constantFieldMap.get(fieldKey);
                            if (constantField != null) {
                                if (formatInfo != null
                                        && formatInfo.getTypeInfo() == StringTypeInfo.INSTANCE) {
                                    inputField =
                                            new StringConstantParam(constantField.getFieldValue());
                                } else {
                                    inputField = new ConstantParam(constantField.getFieldValue());
                            ...
                            FieldInfo outputField =
                                    new FieldInfo(streamField.getFieldName(), formatInfo);
                            return new FieldRelation(inputField, outputField);
                        })
                .collect(Collectors.toList());
    }
    ...
    private static List<FieldRelation> createSplitterFieldRelations(
        ...
        List<FieldRelation> fieldRelations =
                splitRules.stream()
                        .map(
                                splitRule ->
                                        parseSplitRule(
                                                splitRule, splitFields, transformName, preNode))
                        .reduce(
                                Lists.newArrayList(),
                                (list1, list2) -> {
                                    list1.addAll(list2);
        ...
        List<StreamField> filteredFieldList =
                fieldList.stream()
                        .filter(
                                streamFieldInfo ->
                                        !splitFields.contains(streamFieldInfo.getFieldName()))
                        .collect(Collectors.toList());
        ...
        List<FieldRelation> fieldRelations =
                replaceRules.stream()
                        .map(
                                replaceRule ->
                                        parseReplaceRule(
                                                replaceRule, replaceFields, transformName, preNode))
                        .collect(Collectors.toList());
    ...
    private static List<FieldRelation> createEncryptFieldRelations(
            List<StreamField> fieldList,
            String transformName,
            EncryptDefinition encryptDefinition,
            String preNodes,
            Map<String, StreamField> constantFieldMap) {
        ...
        List<FieldRelation> fieldRelations =
                encryptRules.stream()
                        .map(
                                replaceRule ->
                                        parseEncryptRule(
                                                replaceRule, encryptFields, transformName, preNode))
                        .collect(Collectors.toList());
        List<StreamField> filteredFieldList =
                fieldList.stream()
                        .filter(
                                streamFieldInfo ->
                                        !encryptFields.contains(streamFieldInfo.getFieldName()))
                        .collect(Collectors.toList());
        fieldRelations.addAll(createFieldRelations(filteredFieldList, constantFieldMap));
        return fieldRelations;
    }
    ...
    private static List<FieldRelation> cascadeFunctionRelations(
            List<FieldRelation> fieldRelations) {
        ...
        for (FieldRelation fieldRelation : fieldRelations) {
            ...
            cascadeFunctions
                    .computeIfAbsent(targetField, k -> Lists.newArrayList())
                    .add(cascadeFunction);
    ...
    private static FieldRelation parseReplaceRule(
        ...
        if (replaceMode == ReplaceMode.RELACE_ALL) {
            RegexpReplaceFunction regexpReplaceFunction =
                    new RegexpReplaceFunction(
                            fieldInfo,
                            new StringConstantParam(regex),
                            new StringConstantParam(targetValue));
            return new FieldRelation(regexpReplaceFunction, targetFieldInfo);
        } else {
            RegexpReplaceFirstFunction regexpReplaceFirstFunction =
    ...
    private static FieldRelation parseEncryptRule(
        ...
        EncryptFunction encryptFunction =
                new EncryptFunction(
                        fieldInfo, new StringConstantParam(key), new StringConstantParam(encrypt));
        return new FieldRelation(encryptFunction, targetFieldInfo);
    }

    /** Parse rule of split. */
    private static List<FieldRelation> parseSplitRule(
            SplitRule splitRule, Set<String> splitFields, String transformName, String preNode) {
        ...
        for (int index = 0; index < targetSources.size(); index++) {
            SplitIndexFunction splitIndexFunction =
                    new SplitIndexFunction(
                            fieldInfo,
                            new StringConstantParam(separator),
                            new ConstantParam(index));
            FieldInfo targetFieldInfo =
                    new FieldInfo(
                            targetSources.get(index),
                            transformName,
                            FieldInfoUtils.convertFieldFormat(FieldType.STRING.name()));
